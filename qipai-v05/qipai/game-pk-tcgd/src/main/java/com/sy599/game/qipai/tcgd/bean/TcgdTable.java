package com.sy599.game.qipai.tcgd.bean;
...
import com.sy599.game.qipai.tcgd.tool.CardTool;
import com.sy599.game.qipai.tcgd.util.CardUtils;
import com.sy599.game.qipai.tcgd.util.TcgdSfNew;
import com.sy599.game.qipai.tcgd.util.TcgdUtil;
...
public class TcgdTable extends BaseTable {
    public static final String GAME_CODE = "tcgd";
    private static final int JSON_TAG = 1;
    /*** 当前牌桌上出的牌 */
    private volatile List<Integer> nowDisCardIds = new ArrayList<>();
    /*** 玩家map */
    private Map<Long, TcgdPlayer> playerMap = new ConcurrentHashMap<Long, TcgdPlayer>();
    /*** 座位对应的玩家 */
    private Map<Integer, TcgdPlayer> seatMap = new ConcurrentHashMap<Integer, TcgdPlayer>();
    /*** 最大玩家数量 */
    ...
    private volatile int is3daidui = 1;// 1三带对
    private volatile int isFeijiDld = 1;// 1飞机带连对
    private volatile int isZwf = 0; //0,不抓 1 抓尾分
    private volatile int duiwu = 1;//0= 铁队 ；1= 摸队：
    private volatile String showCardsUser;// 明牌玩家
    private volatile int randomFenZuCard = 0;
    private int option_dc2;// 0=红2单出 1=红2不能单出
    private int option_syp; // 0=不显示剩余牌 1 显示
    private int option_baopai;// 爆牌警示灯 7 10;
    private int fapai_startseat;//发牌开始位置
    private volatile long turnfirstseat_userid;//每局首轮出牌玩家ID
    private List<Integer> hongtao3_seat = new ArrayList<>();
    private volatile int deskCardType =0;
    private volatile int isNoticeAllPlay= 1;
    /**
     * 托管1：单局，2：全局
     */
    ...
    private int oldhongtao3seat;
    private boolean needchangeSeat;
    ...
    protected void loadFromDB1(TableInf info) {
        ...
        if (!StringUtils.isBlank(info.getHandPai8())) {
            ...
            for (TcgdPlayer sb : seatMap.values()) {
    ...
    public TcgdPlayer getPlayer(long id) {
    ...
    public void calcOver() {
        ...
        int fristSeat = 0;
        boolean win = false;
        for (TcgdPlayer dp : seatMap.values()) {
            if (dp.getRank() == 1) {
...
//        TcgdPlayer fristPlayer = seatMap.get(fristSeat);
//        int teamSeat = list.get(0);
        int teamSeat =getTeamFriendSeat(fristSeat);
        if(teamSeat==0){
            try {
                LogUtil.msg("===table maybe over===TongChengGuanDan fristSeat="+fristSeat+"|tableId="+getId()+"playBureau="+playBureau+"|table state (1ready 2play 3 over)="+state);
            }catch (Exception e){
                LogUtil.msg("==table maybe over====TongChengGuanDan fristSeat="+fristSeat);
            }
           return;
        }
        TcgdPlayer tp = seatMap.get(teamSeat);
        if (tp.getRank() == 2) {
            //win =12
            for (TcgdPlayer dp : seatMap.values()) {
                if (dp.getSeat() != fristSeat && dp.getSeat() != teamSeat) {
//                    dp.setPmjlfen(-3 * difen);
                    dp.setPoint(-3 * difen);
                    dp.changePlayPoint(-3 * difen);
                } else {
                    dp.setPoint(3 * difen);
                    dp.changePlayPoint(3 * difen);
                }
...
//			 12名是队友是双下情况
//			 局结束后出现双下的情况，则由上局上游的下家在这局中首先摸牌。
            fapai_startseat = getNextPlaySeat(fristSeat);
        } else if (tp.getRank() == 3) {
            //win = 13
            for (TcgdPlayer dp : seatMap.values()) {
                if (dp.getSeat() != fristSeat && dp.getSeat() != teamSeat) {
                    dp.setPoint(-2 * difen);
                    dp.changePlayPoint(-2 * difen);
                } else {
                    dp.setPoint(2 * difen);
                    dp.changePlayPoint(2 * difen);
                }
            ...
            fapai_startseat = tp.getSeat();
            for (TcgdPlayer dp : seatMap.values()) {
                if (dp.getSeat() != fristSeat && dp.getSeat() != teamSeat) {
                    dp.setPoint(-1 * difen);
                    dp.changePlayPoint(-1 * difen);
                } else {
                    dp.setPoint(1 * difen);
                    dp.changePlayPoint(1 * difen);
                }
...
//        for (TcgdPlayer dtp : seatMap.values()) {
//            if (totalBureau >= 100) {
...
//        }
        ...
        if (autoPlayGlob > 0) {
            ...
            boolean diss = false;
            if (autoPlayGlob == 1) {
                for (TcgdPlayer seat : seatMap.values()) {
                    if (seat.isAutoPlay()) {
                        diss = true;
                        break;
                    }
        ...
        for (TcgdPlayer p : seatMap.values()) {
            p.setMyTuoGuanTime(autoTimeOut);
        }
        outfencard.clear();
    }
    ...
    public void calcDataStatistics2() {
        ...
        if (isGroupRoom()) {
            ...
            for (TcgdPlayer player : playerMap.values()) {
                // 总小局数
    ...
    public JsonWrapper buildExtend0(JsonWrapper wrapper) {
        ...
        // 1-4 玩家座位信息
        for (TcgdPlayer player : seatMap.values()) {
        ...
        wrapper.putInt(17, option_dc2);
        wrapper.putInt(18, option_syp);
        wrapper.putInt(19, option_baopai);
        wrapper.putInt(20, fapai_startseat);
        wrapper.putInt(21, tRank);
        wrapper.putInt(22, deskCardType);
        wrapper.putInt(23, isNoticeAllPlay);
//		wrapper.putLong(21, turnfirstseat_userid);
        return wrapper;
    }
    ...
    public void initExtend0(JsonWrapper wrapper) {
        ...
        option_dc2 = wrapper.getInt(17, 0);
        option_syp = wrapper.getInt(18, 0);
        option_baopai = wrapper.getInt(19, 0);
        fapai_startseat = wrapper.getInt(20, 0);
        tRank = wrapper.getInt(21, 0);
        deskCardType = wrapper.getInt(22, 0);
        isNoticeAllPlay =  wrapper.getInt(23, 0);
    }
    ...
    public void fapai() {
        ...
        synchronized (this) {
            ...
            setTableStatus(TcgdConstants.TABLE_STATUS_PLAY);
            timeNum = 0;
            hongtao3_seat.clear();
            List<List<Integer>> fplist = CardTool.fapai(maxPlayerCount, false, zp, false);
//            List<String> sp = TcgdSfNew.getInitArr();
            int randomcard = 303;
            setRandomFenZuCard(randomcard);
            boolean is2 = false;//true=不换位置
            needchangeSeat = false;
            Map<Integer, Integer> randomCardNumMap = new HashMap<>();
            int i = 0;
            int fapai_nextseat = 0;
            if (playBureau == 1) {
                fapai_nextseat = playerMap.get(masterId).getSeat();
            } else {
                fapai_nextseat = fapai_startseat;
            }
            oldhongtao3seat = 0;
            int secendHongtao3seat=0;
            boolean flag = false;
            for (int j = 0; j < 4; j++) {
                TcgdPlayer player = seatMap.get(fapai_nextseat);
                player.changeState(player_state.play);
                //红桃3定队友
                int randomcardnum = CardUtils.getSelTeamCardNum(fplist.get(i), 303);
                player.dealHandPais(fplist.get(i), this);
//                 System.out.println("给" + player.getName() + "  " + fapai_nextseat + " 发牌完毕");
                if (randomcardnum >= 1 && !flag) {
                    turnfirstseat_userid = player.getUserId();
                    oldhongtao3seat = player.getSeat();
                    flag = true;
                }else if(randomcardnum >= 1){
                    secendHongtao3seat = player.getSeat();
                }
                if (randomcardnum == 1) {
                    hongtao3_seat.add(player.getSeat());
                } else if (randomcardnum == 2) {
                    hongtao3_seat.add(player.getSeat());
                    hongtao3_seat.add(player.getSeat());
                }
                ...
                fapai_nextseat = getNextPlaySeat(fapai_nextseat);
            ...
            if (hongtao3_seat.size() == 2 && hongtao3_seat.contains(1) && hongtao3_seat.contains(3)) {
                is2 = true;
            }
            if (hongtao3_seat.size() == 2 && hongtao3_seat.contains(2) && hongtao3_seat.contains(4)) {
                is2 = true;
            }
            //根据发牌情况中含有同一个randomcard 的数量进行位置处理
            if (!is2 && getDuiwu() == 1)  {
                //红桃3位置。
                int seatDuijia = getTeamFriendSeat(oldhongtao3seat);
                    // 交换位置-
                    TcgdPlayer paSec = seatMap.get(secendHongtao3seat);
                    TcgdPlayer paDui =seatMap.get(seatDuijia);

                    seatMap.put(seatDuijia,paSec);
                    seatMap.put(secendHongtao3seat,paDui);

                    paSec.setSeat(seatDuijia);
                    paDui.setSeat(secendHongtao3seat);

                    changeCards(seatDuijia);
                    changeCards(secendHongtao3seat);
                    changePlayers();
            }
            //调整后
            for (TcgdPlayer p : seatMap.values()) {
//                p.writeErrMsg("将调整位置！");
                if (p.getUserId() == turnfirstseat_userid) {
                    turnFirstSeat = p.getSeat();
                    nowDisCardSeat = p.getSeat();
                    setNowDisCardSeat(nowDisCardSeat);
                    banker = p.getSeat();
                }

                //  System.out.println(p.getSeat() + " " + p.getName() + " " + p.getHandPais());
//                p.dealHandPais(p.getHandPais().subList(1, 3), this);
            }
            needchangeSeat = is2;
        }

        for (TcgdPlayer player : seatMap.values()) {
            StringBuilder sb = new StringBuilder("TongChengGuanDan");
    ...
    public TcgdPlayer getPlayerBySeat(int seat) {
     ...
     * @return
     */
    private TcgdPlayer getNextPlayerBySeat(int seat) {
        int next = seat >= maxPlayerCount ? 1 : seat + 1;
    ...
    public CreateTableRes buildCreateTableRes(long userId, boolean isrecover, boolean isLastReady) {
        ...
        synchronized (this) {
            ...
            TcgdPlayer reconnet_player = null;
            for (TcgdPlayer player : playerMap.values()) {
...
//                }
                if (player.getHandPais().isEmpty()) {
                    ...
                    if (!teamSeats.isEmpty()) {
                        TcgdPlayer tp = seatMap.get(teamSeats.get(0));
            ...
            List<Integer> scoreCards = TcgdUtil.getScoreCardsList(getTurnCards());
            if (scoreCards.size() > 0) {
                res.addAllScoreCard(scoreCards);
                int totalScore = TcgdUtil.getScoreCards(getTurnCards());
            ...
            res.addExt(ratio);// 8
            res.addExt(pay);// 9
            res.addExt(lastWinSeat);// 10

            res.addExtStr(String.valueOf(matchId));// 0
            res.addExtStr(cardMarkerToJSON());// 1
            //res.addTimeOut(autoPlay ? autoTimeOut : 0);//
            TcgdPlayer nowplay = seatMap.get(nowDisCardSeat);
            if(null!=nowplay && autoPlay){
                //托管过半
                if(nowplay.getMyTuoGuanTime()==0){
                    nowplay.setMyTuoGuanTime(autoTimeOut);
                }
                res.addTimeOut((int)nowplay.getMyTuoGuanTime());
                long noticetime = lastActionTime+nowplay.getMyTuoGuanTime() -TimeUtil.currentTimeMillis();
                res.addTimeOut(Integer.valueOf((int) noticetime)); //   (( last+60S ) - nowtime);
            }else{
                res.addTimeOut(autoPlay ? autoTimeOut : 0);
                res.addTimeOut(-1);
            }
            ...
            res.addExt(creditCommissionMode1);// 14
            res.addExt(creditCommissionMode2);// 15
            res.addExt(autoPlay ? 1 : 0);// 16
            res.addExt(tableStatus);//17
    ...
    public void disCards(TcgdPlayer player, int action, List<Integer> cards) {
        ...
        int paixAction = action;
        if (action > 10) {
            action = 0;
            deskCardType =paixAction;
        }
        if (action == 0) {
        ...
        // paixAction 11单 12=对子 13=顺 14=连对 15=三张 16= 三代一 17=飞机 20=炸 23同花顺 30=天炸
        if (paixAction > 10) {
            player.setPaitype_(paixAction - 10);
            res.setIsLet(paixAction - 10);
        } else {
            player.setPaitype_(0);
            res.setIsLet(0);
        }
        ...
        // List<Integer> scoreCards = TcgdUtil.getScoreCards2(getTurnCards());
        List<Integer> scoreCards = new ArrayList<>();

        int totalScore = 0;
        // totalScore = TcgdUtil.getScoreCards(getTurnCards());
        ...
        if (action == 0 && player.getHandPais().isEmpty()) {
            ...
            if (tRank >= 2 && isZwf == 0) {
                if (checkFirstSecondTeam() && tRank == 2) {
                    //12名为队友  游戏小结
                    int r3seat = getNextSeat(player.getSeat());
                    rankIncre();
                    TcgdPlayer r3 = seatMap.get(r3seat);
                    r3.setRank(3);
                    for (TcgdPlayer dp : seatMap.values()) {
                        if (dp.getRank() == 0) {
                            rankIncre();
                    ...
                    //双下情况 上局结束后出现双下的情况，则由上局上游的下家在这局中首先摸牌。
                    int fristSeat = 0;
                    for (TcgdPlayer dp : seatMap.values()) {
                        if (dp.getRank() == 1) {
                            fristSeat = dp.getSeat();
                    ...
                    fapai_startseat = getNextSeat(fristSeat);
                    turnOver = true;
                    isOver = true;
                }
//                if (checkFirstThirdTeam() && tRank == 3) {
        ...
        if (!turnOver) {
            if (getActionSeats().isEmpty() || checkTiqianOver(player, action, guos)) {// 一轮打完
                turnOver = true;
                clearPaiType();//清理上手打出牌型
                TcgdPlayer winPlayer = seatMap.get(getTurnFirstSeat());
                winseat = winPlayer.getSeat();
                isOver = turnOver(res, winPlayer);
                if (winPlayer.getRank() == 1 || winPlayer.getRank() == 2) {
                    //对家借风-
                    int duijia_seat = getTeamFriendSeat(winseat);
                    setNowDisCardSeat(duijia_seat);
                    if (!isOver) {
                        duijiajiefeng = 1;
                        scoreCards.add(1);
                    }
        ...
        addPlayLog(addSandhPlayLog2(player.getSeat(), action, chuPaiCards, turnOver, paixAction > 10 ? paixAction - 10 : 0, scoreCards,
        ...
        res.setIsPlay(2);
        res.setIsFirstOut(duijiajiefeng);//对家借风
        int so = (int)seatMap.get(getNowDisCardSeat()).getMyTuoGuanTime();
        if(so ==0){
            so = autoTimeOut;
        }
        res.setCurScore(so);
        ...
        if (seeTeamHand > 0) {
            TcgdPlayer tp = seatMap.get(seeTeamHand);
            ComRes.Builder builder = SendMsgUtil.buildComRes(WebSocketMsgType.DUIYOU_HAND, tp.getHandPais());
            player.writeSocket(builder.build());
        }

        TcgdPlayer p1 = seatMap.get(getNowDisCardSeat());
        checkAutoPass(p1,deskCardType,turnOver);
        if (turnOver) {
            checkAutoOutLastCard(p1);
        }
...
//            11单 12=对子 13=顺 14=连对 15=三张 16=三带对 17=飞机 20=炸 23同花顺 30=天炸
    ...
    /**
     *  if= 2020-3-12 手里的牌只剩小于4张的情况下，上家出牌大于手牌数，系统直接自己操作，要不起
     *  else  2020-3-18 = 如果下家要不起 5秒自动过
     * @param p1
     */
    private void checkAutoPass(TcgdPlayer p1,int px,boolean turnover){
        if(p1.isAutoPlay()){
            return;
        }
        int p1num =p1.getHandPais().size();
        int desknum =getNowDisCardIds().size();
        if (!p1.getHandPais().isEmpty() && ( desknum>p1num ) && p1num<4 ) {
            try {
                Thread.sleep(500);
                if(p1.getSeat()!=getNowDisCardSeat()){
                    return;
                }
                List<Integer> c = new ArrayList<>();
                LogUtil.msgLog.info("===========TongChengGuanDan|"+getId()+"|"+p1.getName()+"手牌="+p1.getHandPais() +"|桌牌=|"+getNowDisCardIds() +"| 系统直接自动操作，要不起");
                playCommand(p1, 1, c);
            } catch (InterruptedException e) {
                LogUtil.msgLog.info(p1.getSeat() + "手里的牌只剩小于4张的情况下，上家出牌大于手牌数，系统直接自己操作，要不起 发生error");
                e.printStackTrace();
            }
        }else{
            if(turnover){
                p1.setIsOutWhenAuto(1);
                LogUtil.msgLog.info("===========TongChengGuanDan|"+getId()+"| 回合结束 不自动检查出要不起。");
                return;
            }

                final long tableid = getId();
                if(getNowDisCardIds().isEmpty()){
                    return;
                }
            final List<Integer> deskCards = TcgdUtil.haveRed2CardsTurn(getNowDisCardIds(),px);
            if(null==deskCards|| deskCards.isEmpty()||deskCards.size()==0){
                return;
            }
            final boolean ts1 = CardUtils.getTs(deskCards,px,p1.getHandPais());
            LogUtil.msgLog.info("===========TongChengGuanDan|"+getId()+"|转换后当前桌牌=|"+deskCards+"|牌型="+px+"->"+getPx(px)+"| 下家="+p1.getName()+"|手牌="+p1.getHandPais());
            LogUtil.msgLog.info("" +"|能否打得起="+ts1);
            if(!ts1){
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            //需要执行的代码
                            try {
                                List<Integer> NewDisCardId = new ArrayList<>(getNowDisCardIds());
                                if(NewDisCardId.isEmpty()){
                                    return;
                                }

                                final String cards1 = NewDisCardId.toString();
                                p1.setIsOutWhenAuto(0);//
                                Thread.currentThread().sleep(5000);

                                TcgdTable table = p1.getPlayingTable(TcgdTable.class);
                                if (table == null) {
                                    LogUtil.msgLog.info("===========TongChengGuanDan|不自动检查 牌局已解散 ");
                                    return;
                                }
                                if (table.getState() != table_state.play) {
                                    LogUtil.msgLog.info("===========TongChengGuanDan|不自动检查 牌局不是play状态");
                                    return;
                                }
                                if(p1.getSeat()!=getNowDisCardSeat()){
                                    LogUtil.msgLog.info("===========TongChengGuanDan|不自动检查 当前出牌人不是"+p1.getName()+"|seat="+p1.getSeat()+"|当前出牌位置"+getNowDisCardSeat());
                                    return;
                                }
                                if(tableid!=getId()){
                                    LogUtil.msgLog.info("===========TongChengGuanDan|不自动检查 牌局已变更");
                                    return;
                                }
                                List<Integer> NewDisCardId2 = new ArrayList<>(getNowDisCardIds());
                                if(NewDisCardId2.isEmpty()){
                                    return;
                                }
                                if(p1.getIsOutWhenAuto()==1){
                                    LogUtil.msgLog.info("===========TongChengGuanDan|不自动检查 5S时间中出过牌。");
                                    return;
                                }

                                if(!NewDisCardId.containsAll(NewDisCardId2)){
                                    LogUtil.msgLog.info("===========TongChengGuanDan|"+getId()+" cards1 not equal card2|cards1="+cards1+"|cards2="+NewDisCardId2);
                                    return;
                                }

                                if(!ts1){
                                    List<Integer> c = new ArrayList<>();
                                    LogUtil.msgLog.info("===========TongChengGuanDan|"+getId()+"|auto pass start= |"+p1.getName()+"|seat="+p1.getSeat());
                                    playCommandAuto(p1,1,c);
                                    p1.setAutoPlay(false, table);
                                    p1.setLastOperateTime(System.currentTimeMillis());
                                    LogUtil.msgLog.info("===========TongChengGuanDan|"+getId()+"|auto pass over= |"+p1.getName()+"|seat="+p1.getSeat());
                                }else{
                                    return;
                                }
                            } catch (InterruptedException e) {
                                LogUtil.msgLog.info("===========TongChengGuanDan|"+getId()+"  nextseat="+p1.getSeat()+ " auto pass error! ");
                                e.printStackTrace();
                            }
                        } }).start();
                }
    ...
    /**
     *   2020-3-12  最后一张 主动出的可以直接出完的牌系统自动出
     * @param p1
     */
    private void checkAutoOutLastCard(TcgdPlayer p1){
        // new require
        if (p1.getHandPais().size() == 1) {
            //11=单
            List<Integer> c = new ArrayList<>(p1.getHandPais());
            playCommand(p1, 11, c);
        }
    }
    private boolean checkTiqianOver(TcgdPlayer player, int action, int guos) {
        if (checkFirstSecondTeam()) {
            TcgdPlayer secondP = getPlayerByRank(tRank);
    ...
    private int getTeamFriendSeat(int banker) {
        if (banker == 1) {
            return 3;
        } else if (banker == 2) {
            return 4;
    ...
    private int getDuijiaSeat(int banker) {
        if (banker == 1) {
            return 3;
        } else if (banker == 2) {
            return 4;
    ...
    private boolean checkFirstSecondTeam() {
        ...
        int firstSeat = 0;
        for (TcgdPlayer splayer : seatMap.values()) {
        ...
        TcgdPlayer pl = seatMap.get(list.get(0));
    ...
    private int chupai(TcgdPlayer player, int action, List<Integer> cards) {
        ...
        // int nextSeat = getNextDisCardSeat();
        //癞子 红桃3 R3 303
//        List<Integer> r3list = new ArrayList<>();//红桃3的下标数组
...
//                for()
//                copy.set(r3list.get(i),);
//            }
//        }
        List<String> reslutList = new ArrayList<>();
        reslutList = TcgdUtil.CheckR3OutCard(TcgdSfNew.listToString(TcgdSfNew.intCardToStringCard(cards)), reslutList);
//     String type = TcgdSfNew.getCpType2(TcgdSfNew.intCardToStringCard(cards), is3daidui, isFeijiDld);
...
//        }
        ...
        List<Integer> scoreCards = TcgdUtil.getScoreCardsList(cards);
...
//		}
    ...
    //	private int checkXifen(TcgdPlayer player, CardType ct) {
    private int checkXifen(TcgdPlayer player) {
        int xifen = 0;
...
//		}
    ...
    //一轮回合结束
    private boolean turnOver(PlayCardRes.Builder res, TcgdPlayer winPlayer) {
        List<Integer> scoreCards = TcgdUtil.getScoreCardsList(getTurnCards());
        if (scoreCards.size() > 0) {
            res.addAllScoreCard(scoreCards);
            winPlayer.addChiFenCards(scoreCards);
            int totalScore = TcgdUtil.loadCardScore(scoreCards);
            ...
            if (!teamSeats.isEmpty()) {
                for (Integer seat : teamSeats) {
                    TcgdPlayer tp = seatMap.get(seat);
                    if (tp != null) {
...
//		}
        ...
        return checkOverGameFen(winPlayer);
    }

    private boolean checkOverGameFen(TcgdPlayer tcgdPlayer) {
        ...
        if (tRank == 2) {
            ...
            rankIncre();
            int totalScore = 0;
            TcgdPlayer first = getPlayerByRank(1);
            for (TcgdPlayer p : seatMap.values()) {
                if (p.getRank() == 0) {
                    //4
                    p.setRank(tRank);
                    fapai_startseat = p.getSeat();
                    break;
                }
            }
            return true;
        } else if (tRank == 4) {
            fapai_startseat = tcgdPlayer.getSeat();
            return true;
        }
    ...
    private int getNextPlaySeat(int nextSeat) {
        for (int i = 0; i < maxPlayerCount - 1; i++) {
            ...
            TcgdPlayer nextPlayer = seatMap.get(nextSeat);
    ...
    private TcgdPlayer getPlayerByRank(int rank) {
        TcgdPlayer player = null;
        for (TcgdPlayer p : seatMap.values()) {
            if (p.getRank() == rank) {
                player = p;
            }
    ...
    private void clearPaiType() {
        for (TcgdPlayer p : playerMap.values()) {
            p.setPaitype_(0);
        }
    }
     ...
     * @param cards
     */
    public void playCommand(TcgdPlayer player, int action, List<Integer> cards) {
        synchronized (this) {
            if (state != table_state.play) {
                return;
            }
//			// 出牌阶段
//			if (getTableStatus() != TcgdConstants.TABLE_STATUS_PLAY) {
//				return;
//			}
            ...
            if (option_dc2 == 1 && cards.size() == 1 && cards.get(0) == 315 && player.getHandPais().size() > 2) {
                player.writeErrMsg("红桃2不能单出！");
                return;
            }
            ...
            if(player.getIstuoguanTimeJianBan()==0){
                //0 还没减半 计算时间
                long noticetime = lastActionTime+autoTimeOut -TimeUtil.currentTimeMillis();
                //出牌时间>托管过半时间。 自己托管减半
                if(noticetime<(autoTimeOut/2)){
                    player.setIstuoguanTimeJianBan(1);
                    int autoTimeout = autoTimeOut/2;
                    if(autoTimeout<=30000){
                        autoTimeout=30000;
                    }
                    player.setMyTuoGuanTime(autoTimeout);
                }
            }else if(player.getIstuoguanTimeJianBan()==1){
                //已经托管减半过了
                long noticetime = lastActionTime+player.getMyTuoGuanTime() -TimeUtil.currentTimeMillis();
                if(noticetime<(player.getMyTuoGuanTime()/2)){
                    long autoTimeout = player.getMyTuoGuanTime()/2;
                    if(autoTimeout<=30000){
                        autoTimeout=30000;
                    }
                    player.setMyTuoGuanTime(autoTimeout);
            ...
            setLastActionTime(TimeUtil.currentTimeMillis());
            player.setIsOutWhenAuto(1);//在新增5S自动过时间 出过牌
            player.setLastOperateTime(System.currentTimeMillis());
            if (isOver()) {
                calcOver();
            }
    ...
    public void playCommandAuto(TcgdPlayer player, int action, List<Integer> cards) {
        synchronized (this) {
             player.setIsOutWhenAuto(1);//在新增5S自动过时间 出过牌
            if (state != table_state.play) {
                return;
...
//		json.put("xifen", nextSeat);
    ...
    public void playChuPaiRecord(TcgdPlayer player) {
        JSONArray jarr = new JSONArray();
        for (TcgdPlayer splayer : seatMap.values()) {
            JSONObject json = new JSONObject();
            json.put("seat", splayer.getSeat());
    ...
    public void playFenCards(TcgdPlayer player, int type) {
        if (state != table_state.play) {
    ...
    public void playShowCards(TcgdPlayer player) {
        if (player.isShowCards() == 1) {
            return;
        }
        //action =6  玩家明牌
        addPlayLog(addSandhPlayLog2(player.getSeat(), TcgdConstants.MINGPAI, null, false, 0, null,
...
//					WebSocketMsgType
    ...
    public void allocTeam(int bankerSeat) {
        ...
        addPlayLog(addSandhPlayLog(0, TcgdConstants.FENZU, teamSeat, false, 0, null, getNowDisCardSeat()));

        StringBuilder sb = new StringBuilder("TongChengGuanDan");
        sb.append("|").append(getId());
        sb.append("|").append(getPlayBureau());
        sb.append("|").append("fenzu");
        sb.append("|").append(teamSeat);
        LogUtil.msgLog.info(sb.toString());
    }
    ...
    protected void sendDealMsg(long userId) {
        //发牌
        for (TcgdPlayer tablePlayer : seatMap.values()) {
            DealInfoRes.Builder res = DealInfoRes.newBuilder();
            res.addAllHandCardIds(tablePlayer.getHandPais());
            // res.setNextSeat(nowDisCardSeat);//
            res.setNextSeat(oldhongtao3seat);
            res.setGameType(getWanFa());//i
            res.setRemain(2);//发牌之前 0.  打牌2
            tablePlayer.writeSocket(res.build());
            if (tablePlayer.isAutoPlay()) {
                ArrayList<Integer> val = new ArrayList<>();
                val.add(1);
                addPlayLog(addSandhPlayLog(tablePlayer.getSeat(), TcgdConstants.action_tuoguan, val, false, 0, null,
                        0));
            }
        }
        //红桃3消息
        final ComRes.Builder builder = SendMsgUtil.buildComRes(WebSocketMsgType.REQ_TCGD_HONGTAO3, hongtao3_seat);
        for (TcgdPlayer splayer : seatMap.values()) {
            splayer.writeSocket(builder.build());
        }

        //换位置消息
        if (!needchangeSeat && getDuiwu() == 1) {
            JSONObject seat_json = new JSONObject();
            for (TcgdPlayer p : seatMap.values()) {
                seat_json.put(p.getUserId() + "", p.getSeat());
            }
            ComRes.Builder builder2 = SendMsgUtil.buildComRes(WebSocketMsgType.REQ_TCGD_NEW_SEAT, seat_json.toJSONString());
            for (TcgdPlayer splayer : seatMap.values()) {
    ...
    public boolean createTable(Player player, int play, int bureauCount, List<Integer> params, boolean saveDb)
        ...
        payType = StringUtil.getIntValue(params, 2, 1);// 1AA,2房主
        difen = StringUtil.getIntValue(params, 3, 1);//
        is3daidui = 1;//StringUtil.getIntValue(params, 4, 0);//
        isFeijiDld = 1;// StringUtil.getIntValue(params, 5, 0);// 飞机带连队
        isZwf = 0;// = StringUtil.getIntValue(params, 6, 0);//抓尾分
        maxPlayerCount = StringUtil.getIntValue(params, 7, 4);// 人数
        duiwu = 1;//= StringUtil.getIntValue(params, 8, 0);//modui =2,zuodui=1
        option_dc2 = StringUtil.getIntValue(params, 11, 0);// 0=红2单出 1=红2不能单出
        option_syp = StringUtil.getIntValue(params, 12, 0);// 0=不显示剩余牌 1显示
        option_baopai = StringUtil.getIntValue(params, 13, 0);// 7 10
        if (maxPlayerCount == 0) {
            maxPlayerCount = 4;
        }
    ...
    public ClosingInfoRes.Builder sendAccountsMsg(boolean over, boolean isBreak, int firstSeat, boolean win) {
        ...
        if (isCreditTable()) {
            ...
            for (ClosingPlayerInfoRes.Builder builder : builderList) {
                TcgdPlayer player = seatMap.get(builder.getSeat());
...
//			builder.addExt(player.getPiaoFen() + ""); // 13
    ...
    public Class<? extends Player> getPlayerClass() {
        return TcgdPlayer.class;
    }

    @Override
    public int getWanFa() {
        return GameUtil.play_type_pk_tcgd;
    }
    ...
    public void checkAutoPlay() {
        synchronized (this) {
//            1、有人进入托管，自动解散该局，进入结算，战绩标明为托管解散，托管玩家出3分，其他玩家+1分（第一局只解散，不算分）
//            2、在托管时间倒计时进入到一半的时候，系统弹出漂浮框（以托管时间为2分钟举例）
//            ：A玩家已经超过1分钟未出牌，超过2分钟牌局会自动解散，A玩家-3分，其他玩家每人+1分，请理解并耐心等待。（第一局只解散不算分）
//            if (state == table_state.play) {
////                if (getTableStatus() == TcgdConstants.TABLE_STATUS_PLAY) {
////                    TcgdPlayer nowPlay = seatMap.get(nowDisCardSeat);
////                    if(null!= nowPlay){
////                        if(isNoticeAllPlay==0){
////
////                            long halftime = autoTimeOut/2;
////                            long nowtime =TimeUtil.currentTimeMillis();
////                            long lastOperateTime =  lastActionTime;
////                            //4秒发牌时间。
////                            if(nowtime>(lastOperateTime+halftime)){
////                                setIsNoticeAllPlay(1);
////                                ComRes.Builder builder = SendMsgUtil.buildComRes(WebSocketMsgType.REQ_TCGD_TUOGUAN_NOTICE, nowPlay.getSeat(),nowPlay.getName());
////                                for (TcgdPlayer spl:seatMap.values()) {
////                                    spl.writeSocket(builder.build());
////                                }
////                            }
////                        }
////                    }
...
////            }
            ...
            if (isAutoPlayOff()) {
                ...
                for (int seat : seatMap.keySet()) {
                    TcgdPlayer player = seatMap.get(seat);
                    player.setAutoPlay(false, this);
                }
                return;
            }
            ...
            TcgdPlayer player = seatMap.get(nowDisCardSeat);
            if (player == null) {
                return;
            }
            ...
            //托管减半时间计算
            boolean autoJianban =false;
            
            if(player.getIstuoguanTimeJianBan()==1){
                long  time1 = lastActionTime+player.getMyTuoGuanTime();
            ...
            if (auto || player.isRobot()||autoJianban) {
...
//                        } else {
//                            //红桃2不能单出情况处理
//                            if (curList.get(0) == 315 && curList.size() >= 2 && curList.get(1) < 500) {
//                                disList.add(curList.get(0));
//                                disList.add(curList.get(1));
//                            } else if (curList.get(0) == 315 && curList.size() >= 2 && curList.get(1) > 500) {
//                                disList.add(curList.get(1));
//                            } else {
//                                disList.add(curList.get(0));
//                            }
...
//                        }
    ...
    private void AutoDissRoom(TcgdPlayer nowPlay) {
        //nowPlay -3
        //elsePlay +1;
    ...
    public boolean checkPlayerAuto(TcgdPlayer player, int timeout) {
        if (player.isAutoPlay()) {
            return true;
        }
        long now = TimeUtil.currentTimeMillis();
    ...
    private String cardMarkerToJSON() {
        ...
        for (Map.Entry<Integer, TcgdPlayer> entry : seatMap.entrySet()) {
    ...
    public static final List<Integer> wanfaList = Arrays.asList(GameUtil.play_type_pk_tcgd);//GameUtil.play_type_pk_Tcgd=255
...
//         return tlist;
    ...
    public void setPlayerMap(Map<Long, TcgdPlayer> p) {
        this.playerMap = p;
    }

    public void setSeatMap(Map<Integer, TcgdPlayer> map) {
        this.seatMap = map;
    }
    ...
    public int getOption_dc2() {
        return option_dc2;
    }

    public void setOption_dc2(int option_dc2) {
        this.option_dc2 = option_dc2;
    }

    public int getOption_syp() {
        return option_syp;
    }

    public boolean isshow_syp() {
        return option_syp == 1;
    }

    public void setOption_syp(int option_syp) {
        this.option_syp = option_syp;
    }
    ...
    public int getIsNoticeAllPlay() {
        return isNoticeAllPlay;
    }

    public void setIsNoticeAllPlay(int isNoticeAllPlay) {
        this.isNoticeAllPlay = isNoticeAllPlay;
        changeExtend();
    }
}
