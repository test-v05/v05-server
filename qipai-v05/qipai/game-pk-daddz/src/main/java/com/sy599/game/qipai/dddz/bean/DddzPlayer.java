...
import com.sy599.game.qipai.dddz.command.DddzCommandProcessor;
...
public class DddzPlayer extends Player {
	...
	protected void loadFromDB0(RegInfo info) {
		if (UserDatasDao.getInstance().exists()){
			String val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId),DddzTable.GAME_CODE,"all","currentLs");
			if (CommonUtil.isPureNumber(val)){
				currentLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId),DddzTable.GAME_CODE,"all","currentLs","0");
			}

			val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId),DddzTable.GAME_CODE,"all","maxLs");
			if (CommonUtil.isPureNumber(val)){
				maxLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId),DddzTable.GAME_CODE,"all","maxLs","0");
			}
		}
	}
	...
	public void setAutoPlay(boolean autoPlay,BaseTable table) {
		if (this.autoPlay != autoPlay && !isRobot()){
			...
			DddzTable table2 = getPlayingTable(DddzTable.class);
			...
			if(null == table2){
				...
				table2.addPlayLog(table2.addSandhPlayLog(getSeat(), DddzConstants.action_tuoguan, val, false, 0, null, 0));
			}
        	// table.addPlayLog(getSeat(), DddzConstants.action_tuoguan + "",(autoPlay?1:0) + "");
		}
	...
	public void dealHandPais(List<Integer> pais,DddzTable table) {
		this.table = table;
		this.handPais = pais;
	...
	public void addOutPais(List<Integer> cards ,DddzTable table) {
		this.table = table;
	...
	public PlayerInTableRes.Builder buildPlayInTableInfo(long recUserId, boolean isrecover) {
		...
		DddzTable table = getPlayingTable(DddzTable.class);
		if (table == null) {
	...
	public ClosingPlayerInfoRes.Builder bulidOneClosingPlayerInfoRes() {
		...
		res.setSeat(seat);
		res.setIsHu(getPlayPoint()>0?1:0);
		DddzTable table = getPlayingTable(DddzTable.class);
	...
	public static void loadWanfaPlayers(Class<? extends Player> cls){
		for (Integer integer:wanfaList){
			PlayerManager.wanfaPlayerTypesPut(integer,cls, DddzCommandProcessor.getInstance());
		}
	}
