...
import com.sy599.game.qipai.hylhq.command.PaohuziCommandProcessor;
import com.sy599.game.qipai.hylhq.constant.EnumHelper;
...
public class HylhqPlayer extends Player {
    // 座位id
    private int seat;
    // 状态
...
//        if(!isNeedDui)
//            return null;
        HylhqTable table = getPlayingTable(HylhqTable.class);
        //这个起手提需要关注一下，改错了
        ...
        if (action == PaohzDisAction.action_ti||action== PaohzDisAction.action_pao)
            isNeedDui=true;
        List<PaohuziHuLack> list = PaohuziTool.isHuNew1(PaohuziTool.getPaohuziHandCardBean(handCopy), disCard, isSelfMo,  isNeedDui, true,table.getVal1_5_10()==1);
    ...
    public PaohuziHuLack checkHuNew(PaohzCard card, boolean isSelfMo) {
        ...
        HylhqTable table = getPlayingTable(HylhqTable.class);
        List<PaohuziHuLack> list = PaohuziTool.isHuNew1(PaohuziTool.getPaohuziHandCardBean(handCopy), card, isSelfMo, isNeedDui(), isPaoHu,table.getVal1_5_10()==1);
    ...
    public PaohuziCheckCardBean  checkCard(PaohzCard card, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass) {
        ...
        if (card != null) {
            ...
            if (isCanChiPeng && !check.isTi() && !check.isPao() && isCheckChi && !isPass) {
                ...
                HylhqPlayer frontPlayer = (HylhqPlayer) table.getPlayerBySeat(table.calcFrontSeat(seat));
                ...
                if (!passChiList.contains(card.getVal()) && !passChi.contains(card.getVal())) {
                    List<PaohzCard> chiList = getChiList(card, null,table.getVal1_5_10()==1);
...
//    public List<PhzHuCards> buildPhzCards(HylhqTable table,long lookUid) {
//        List<PhzHuCards> list = new ArrayList<>();
//        for (CardTypeHuxi type : cardTypes) {
//            if (type.getAction() == PaohzDisAction.action_zai) {
//                // 不是本人并且是栽
...
//    }
    ...
    public List<PhzHuCards> buildPhzCards(HylhqTable table,long someOne) {
        List<PhzHuCards> list = new ArrayList<>();
        for (CardTypeHuxi type : cardTypes) {
