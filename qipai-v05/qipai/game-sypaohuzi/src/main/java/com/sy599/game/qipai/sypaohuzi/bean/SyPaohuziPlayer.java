...
import com.sy599.game.qipai.sypaohuzi.command.PaohuziCommandProcessor;
import com.sy599.game.qipai.sypaohuzi.constant.EnumHelper;
...
public class SyPaohuziPlayer extends Player {
	// 座位id
	private int seat;
	// 状态
...
//        }
	...
	public PaohuziHuLack checkPaoHu(PaohzCard card, boolean isSelfMo, boolean canDiHu) {
		...
		if (card != null) {
			SyPaohuziTable table = getPlayingTable(SyPaohuziTable.class);
	...
	public PaohuziHuLack checkHu(PaohzCard card, boolean isSelfMo) {
		...
		if (lack.isHu()) {
			if (lack.getHuxi() + outHuXi + zaiHuxi < 10) {
				// 10胡息能胡
				lack.setHu(false);
			} else {
				setHuxi(lack.getHuxi());
	...
	public PaohuziCheckCardBean checkCard(PaohzCard card, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass) {
		...
		if (card != null) {
			...
			if (isCanChiPeng && !check.isTi() && !check.isPao() && isCheckChi && !isPass) {
				...
				SyPaohuziPlayer frontPlayer = (SyPaohuziPlayer) table.getPlayerBySeat(table.calcFrontSeat(seat));
	...
	public PlayerInTableRes.Builder buildPlayInTableInfo(long lookUid, boolean isrecover) {
		...
		if (seat==table.getShuXingSeat()){
			SyPaohuziPlayer paohuziPlayer = (SyPaohuziPlayer)table.getSeatMap().get(table.getLastWinSeat());
	...
	public List<PhzHuCards> buildPhzCards(SyPaohuziTable table,long lookUid) {
		List<PhzHuCards> list = new ArrayList<>();
		for (CardTypeHuxi type : cardTypes) {
			if (type.getAction() == PaohzDisAction.action_zai) {
				// 不是本人并且是栽
