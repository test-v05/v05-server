...
//                setToCache(user);
	 ...
	 * @return
	 */
	public long changeUserCards(long userId, long cards, int sourceType) {
		return changeUserCards(userId, cards, sourceType, 1);
	}

	private long changeUserCards(long userId, long cards, int sourceType, int tryCount) {
		long[] longs = loadUserCard(userId);
		long selfCards = longs[0];
		long selfFreeCards = longs[1];
		long oldCards = selfCards;
		...
		if (cards < 0) {
			...
			if (temp >= 0) {
				...
				if(sourceType == SourceType.admin_change_2.type()){
					// 不够扣除时，直接清空
					minusCards = selfCards;
					selfCards = 0;
				}else{
		...
		if (minusCards != 0) {
			if (changeUserCard(userId, oldCards, oldFreeCards, -minusCards, 0) <= 0) {
				if (tryCount++ > 5) {
					LogUtil.i("changeUserCards|fail|" + userId + "|" + cards + "|" + sourceType + "|" + tryCount);
					return 0;
				}
				return changeUserCards(userId, cards, sourceType, tryCount);
			} else {
				LogUtil.i("changeUserCards|succ|" + userId + "|" + "|" + cards + "|" + sourceType + "|" + tryCount);

				UserCardRecordInfo record = new UserCardRecordInfo(userId, selfFreeCards, selfCards, 0, (int) -minusCards, sourceType);
				UserCardRecordDao.getInstance().insert(record);
				try {
					RegInfo regInfo = UserDao.getInstance().getUser(userId);
					if (regInfo != null && regInfo.getEnterServer() > 0 && regInfo.getIsOnLine() == 1) {
						GameUtil.notifyChangCards(regInfo.getEnterServer(), userId, -minusCards, 0, false);
					}
				} catch (SQLException e) {
					e.printStackTrace();
				}
	 ...
	 * @return
	 */
	public long changeUserFreeCards(long userId, long freeCards, int sourceType) {
		return changeUserFreeCards(userId, freeCards, sourceType, 1);
	}

	private long changeUserFreeCards(long userId, long freeCards, int sourceType, int tryCount) {
		long[] longs = loadUserCard(userId);
		long selfCards = longs[0];
		long selfFreeCards = longs[1];
		long oldCards = selfCards;
		...
		if (minusFreeCards != 0) {
			if (changeUserCard(userId, oldCards, oldFreeCards, 0, -minusFreeCards) <= 0) {
				if (tryCount++ > 5) {
					LogUtil.i("changeUserFreeCards|fail|" + userId + "|" + freeCards + "|" + sourceType + "|" + tryCount);
					return 0;
				}
				return changeUserFreeCards(userId, freeCards, sourceType, tryCount);
			} else {
				LogUtil.i("changeUserFreeCards|succ|" + userId + "|" + "|" + freeCards + "|" + sourceType + "|" + tryCount);

				UserCardRecordInfo record = new UserCardRecordInfo(userId, selfFreeCards, selfCards, (int) -minusFreeCards, 0, sourceType);
				UserCardRecordDao.getInstance().insert(record);
				try {
					RegInfo regInfo = UserDao.getInstance().getUser(userId);
					if (regInfo != null && regInfo.getEnterServer() > 0 && regInfo.getIsOnLine() == 1) {
						GameUtil.notifyChangCards(regInfo.getEnterServer(), userId, 0, -minusFreeCards, false);
					}
				} catch (SQLException e) {
					e.printStackTrace();
				}
