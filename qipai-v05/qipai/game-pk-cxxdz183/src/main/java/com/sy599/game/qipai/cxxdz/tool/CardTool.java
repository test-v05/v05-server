package com.sy599.game.qipai.cxxdz.tool;

import com.sy599.game.GameServerConfig;
import com.sy599.game.qipai.cxxdz.constant.CxxdzConstants;
import com.sy599.game.qipai.cxxdz.util.CardType;
...
public final class CardTool {
	...
	public static List<List<Integer>> fapai(int playerCount, List<List<Integer>> zps) {
		...
		List<Integer> copy = new ArrayList<>(CxxdzConstants.cardList);
		...
		if (GameServerConfig.isDebug()) {
			if (zps != null && !zps.isEmpty()) {
				...
				for (List<Integer> zp : zps) {
				 ...
				 copy = new ArrayList<>(CxxdzConstants.cardList);
				for(Integer id: pai2){
					copy.remove(id);
				}
				list.addAll(zps);
		...
		list.add(pai);
	 	list.add(dipai);
		int  listindex =new Random().nextInt(2);
		int  tipcard = list.get(listindex).get(new Random().nextInt(list.size()-1));
		List<Integer> pai2 = new ArrayList<>();
		pai2.add(tipcard);
		list.add(pai2);
