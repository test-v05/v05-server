...
public class JhPhzCardTypeHuxi {
	private int action;
	private List<Integer> cardIds;
	private int hux;

	public JhPhzCardTypeHuxi() {
	}

	public JhPhzCardTypeHuxi(int action, List<Integer> cardIds, int hux) {
		this.action = action;
	...
	public Builder buildMsg(boolean hideCards) {
		...
		if (hideCards) {
			msg.addAllCards(JhPhzConstants.toPhzCardZeroIds(cardIds));
		} else {
