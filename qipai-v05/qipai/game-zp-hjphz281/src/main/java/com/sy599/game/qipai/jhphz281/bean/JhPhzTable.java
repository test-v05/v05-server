package com.sy599.game.qipai.jhphz281.bean;

import static com.sy599.game.qipai.jhphz281.util.JhPhzConstants.action_chi;
import static com.sy599.game.qipai.jhphz281.util.JhPhzConstants.action_hu;
import static com.sy599.game.qipai.jhphz281.util.JhPhzConstants.action_pao;
import static com.sy599.game.qipai.jhphz281.util.JhPhzConstants.action_pass;
import static com.sy599.game.qipai.jhphz281.util.JhPhzConstants.action_peng;
import static com.sy599.game.qipai.jhphz281.util.JhPhzConstants.action_wangchuang;
import static com.sy599.game.qipai.jhphz281.util.JhPhzConstants.action_wangdiao;
import static com.sy599.game.qipai.jhphz281.util.JhPhzConstants.action_wangzha;
...
import com.sy599.game.qipai.jhphz281.util.GuihzCard;
...
import com.sy599.game.qipai.jhphz281.util.JhPhzFanEnums;
...
public class JhPhzTable extends BaseTable implements JhPhzBase {
	/*** 玩家map */
    private Map<Long, JhPhzPlayer> playerMap = new ConcurrentHashMap<>();
	/*** 座位对应的玩家 */
    private Map<Integer, JhPhzPlayer> seatMap = new ConcurrentHashMap<>();
    private List<Integer> leftCards = new ArrayList<>();
	/*** 摸牌flag */
    private volatile int moFlag;
	/*** 应该要打牌的flag */
    private volatile int toPlayCardFlag;
    private volatile JhPhzCheckCardBean autoDisBean;
...
//		JsonWrapper wrapper = new JsonWrapper(info);
        ...
        if (!StringUtils.isBlank(autoDisPhz)) {
            autoDisBean = new JhPhzCheckCardBean();
        ...
        hongHeiDian= wrapper.getInt(35, 0);
        if(tuoguan>0) {
    ...
    private synchronized boolean recallAction(int userSeat, int action, List<Integer> cards) {
        ...
        if (seat0 != -1 && seat0 != userSeat) {
            ...
            if (act != -1 && act == action0) {
				...
				// action 1胡2碰6吃7跑
                if (act == 1) {
                    addAction(seat0, JhPhzCheckCardBean.hasActionList(0));
                } else if (act == 2) {
                    addAction(seat0, JhPhzCheckCardBean.hasActionList(1));
                } else if (act == 6) {
                    addAction(seat0, JhPhzCheckCardBean.hasActionList(4));
                } else if (act == 9) {
                    addAction(seat0, JhPhzCheckCardBean.hasActionList(5));
                }
    ...
    public void calcOver() {
        ...
        int fangPaoSeat =0;
        int fanXinCardId2 =0;
        JhPhzPlayer fangPaoPlayer = null;
        if(!isMoFlag()) {
...
//        }
        ...
        for (int winSeat : winList) {
            ...
            JhPhzPlayer winPlayer = seatMap.get(winSeat);
            int getPoint = 0;
            int onePoint = 0;
           // int temPoint = 0;
            ...
            winPlayer.changeAction(JhPhzConstants.ACTION_COUNT_INDEX_HU, 1);
            if(isSelfMo){
            	winPlayer.changeAction(JhPhzConstants.ACTION_COUNT_INDEX_ZIMO, 1);
            }

            fanxinPoint = processFanXin(winPlayer, fanXinCards);
            totalTun = winPlayer.calcHuPoint(this);
//            totalFan = winPlayer.getTotalFan();
            totalFan = winPlayer.getTotalFan(this.hongHeiDian);
            mingtang = winPlayer.loadFan(this.hongHeiDian);
...
//            }
            ...
            for (int seat : seatMap.keySet()) {
                if (!winList.contains(seat)) {
                    JhPhzPlayer player = seatMap.get(seat);
//                    if (hasZuoXing() && seat == shuXingSeat) {
//                        JhPhzPlayer shuXingPlayer = seatMap.get(shuXingSeat);
...
//            }
        ...
        if (winList.isEmpty()) {
            if (wangbaCount <= 0) {
                for (Entry<Integer, JhPhzPlayer> kv : seatMap.entrySet()) {
...
//                    } else {
                        ...
                        for (Entry<Integer, JhPhzPlayer> kv1 : seatMap.entrySet()) {
...
//                    }
        ...
        if(autoPlayGlob >0) {
            ...
            boolean diss = false;
            if(autoPlayGlob ==1) {
            	 for (JhPhzPlayer seat : seatMap.values()) {
                 	if(seat.isAutoPlay()) {
                     	diss = true;
                     	break;
                     }
        ...
        // 金币场
        if(isGoldRoom()){
            for(JhPhzPlayer player : seatMap.values()){
...
//		}
    ...
    public int processFanXin(JhPhzPlayer player, List<Integer> fanXinCards) {
//        int card = player.getHuResult().getXingCard();
    	 int card = loadXingCard(false);
        fanXinCards.add(card);
        return loadXingTun(player.loadValCount(null, JhPhzCardUtils.loadCardVal(card)));
    }
    ...
    public void saveLog(boolean over, long winId, Object resObject) {
        ...
        if (!isGoldRoom()) {
            for (JhPhzPlayer player : playerMap.values()) {
...
//		int lastCardIndex = RandomUtils.nextInt(21);
        ...
        for (JhPhzPlayer tablePlayer : seatMap.values()) {
...
//            }
        ...
        for (JhPhzPlayer tablePlayer : seatMap.values()) {
        	...
        	if(tablePlayer.isAutoPlay()) {
        		 addPlayLog(tablePlayer.getSeat(), JhPhzConstants.action_tuoguan + "",1 + "");
        	}
    ...
    public void play(JhPhzPlayer player, List<Integer> cardIds, int action) {
        play(player, cardIds, action, false, false, false);
    }

    private synchronized void hu(JhPhzPlayer player, List<Integer> cardList, int action, Integer nowDisCard) {
      ...
      if((action == JhPhzConstants.action_wangdiao || action == JhPhzConstants.action_wangchuang
              || action == JhPhzConstants.action_wangzha)&&!canwang){
    	  return;
      }
...
//            LogUtil.msgLog.info(" checkAction" + tempAction);
        ...
        if (actionList.get(0).intValue() != 1) {
            ...
            if ((action==JhPhzConstants.action_hu||action == JhPhzConstants.action_wangdiao) && actionList.get(7).intValue() == 1) {
                wangHu = 1;
            } else if ((action==JhPhzConstants.action_hu||action == JhPhzConstants.action_wangchuang) && actionList.get(8).intValue() == 1) {
                wangHu = 2;
            } else if ((action==JhPhzConstants.action_hu||action == JhPhzConstants.action_wangzha) && actionList.get(9).intValue() == 1) {
                wangHu = 3;
            } else {
        ...
        if (isPaoHu) {
            int val = JhPhzCardUtils.loadCardVal(huCard);
            ...
            if (list != null) {
                ...
                hu = JhPhzHuPaiUtils.huPai(player.getHandCards(), 0, this, isZiMo, player.getUserId(), loadXingCard(false));
                ...
                hu = JhPhzHuPaiUtils.huPai(player.getHandCards(), huCard, this, isZiMo, player.getUserId(), loadXingCard(false));
...
//                }
                ...
                if (list != null) {
                	int disc = disCardSeat;
                	 play(player, list,JhPhzConstants.action_pao , false, true,false);
...
//            if(isTianHu()) {
//            	 hu.addFan(JhPhzFanEnums.TIAN_HU);
//            	 hu.removeFan(JhPhzFanEnums.SELF);
//            	 hu.removeFan(JhPhzFanEnums.WANG_DIAO);
//            	 hu.removeFan(JhPhzFanEnums.WANG_ZHA);
//            	 hu.removeFan(JhPhzFanEnums.WANG_CHUANG);
//            	 hu.removeFan(JhPhzFanEnums.WANG_CHUANG_WANG);
//            	 hu.removeFan(JhPhzFanEnums.WANG_DIAO_WANG);
//            }else if(isDiHu()) {
//            	hu.addFan(JhPhzFanEnums.DI_HU);
//            }
            if (wangHu <= 0) {
            ...
            sendActionMsg(player, action, null, JhPhzConstants.action_type_action);
            if (diaoOrChuang) {
                clearAction();
                sendActionMsg(player, JhPhzConstants.action_mo, Arrays.asList(huCard), JhPhzConstants.action_type_mo);
            }
	...
	private List<Integer> getPaoPaiList(JhPhzPlayer player, int val) {
	 ...
	 * @return
	 */
    public boolean isSelfMo(JhPhzPlayer player) {
//        if (moSeatPair != null) {
...
//        }
	 ...
	 * @param action
	 */
    private void ti(JhPhzPlayer player, List<Integer> cardList, Integer nowDisCard, int action, boolean moPai) {
		// cards肯定是4个相同的
        if (cardList == null) {
        ...
        int val = JhPhzCardUtils.loadCardVal(cardList.get(0));
        if (cardList.size() == 3) {
...
//			cardList.addAll(tiCards);
        ...
        if (JhPhzCardUtils.loadIdsByVal(cardList, val).size() != 4) {
			// System.out.println("提不合法:" + cardList);
			player.writeErrMsg("提不合法:" + cardList);
            return;
        }
        player.changeAction(JhPhzConstants.ACTION_COUNT_INDEX_TI, 1);
        ...
        JhPhzCheckCardBean checkCard = player.checkCard(0, true, false, false, false, false, true);
...
//            weiOrPao = cardList.get(0);
        ...
        sendActionMsg(player, action, cardList, JhPhzConstants.action_type_action, isZaiPao, false, -1);

        if (checkCard.isHu()) {
        	if(player.getHandCards().WANGS.size()==0) {
        		toPlayCardFlag = 0;
				checkCard.setAuto(JhPhzConstants.action_hu, new ArrayList<>());
    ...
    private boolean siShou(JhPhzPlayer player) {
        JhPhzHandCards handCards = player.getHandCards();
	 ...
	 * @param action
	 */
    private void zai(JhPhzPlayer player, List<Integer> cardList, Integer nowDisCard, int action) {
...
//			System.out.println(actionSeatMap);
        ...
        setBeRemoveCard(nowDisCard);
        if (action == JhPhzConstants.action_zai) {
        ...
        if (checkCard.isHu()) {
        	setHuCard(0);
			setDisposeCard(0);
        	 sendActionMsg(player, action, cardList, JhPhzConstants.action_type_action);
			if (player.getHandCards().WANGS.size()==0) {
				toPlayCardFlag = 0;
				checkCard.setAuto(JhPhzConstants.action_hu, new ArrayList<>());
				setAutoDisBean(checkCard);
...
//			sendActionMsg(player, JhPhzConstants.action_hu, new ArrayList<Integer>(), JhPhzConstants.action_type_mo);
           // sendActionMsg(player, action, cardList, JhPhzConstants.action_type_action);
        } else {
            sendActionMsg(player, action, cardList, JhPhzConstants.action_type_action);
            boolean mo = actionSeatMap.isEmpty() && toPlayCardFlag == 0;
	 ...
	 * @param action
	 */
    private void pao(JhPhzPlayer player, List<Integer> cardList, Integer nowDisCard, int action, boolean isHu, boolean isPassHu) {
        if (cardList.size() != 3) {
...
//        }
        ...
        boolean isZaiPao = player.isZaiPao(JhPhzCardUtils.loadCardVal(cardList.get(0)));
        getDisPlayer().removeOutPais(nowDisCard);
        if (!cardList.contains(nowDisCard)) {
            cardList.add(0, nowDisCard);
        }
        ...
        if (cardList.size() == 1) {
			// 如果是一张牌说明已经在出的牌里面了
//			List<Integer> list = JhPhzCardUtils.loadSameValCards(player.getHandCards().INS,nowDisCard);
//			cardList.addAll(list);
        ...
        player.changeAction(JhPhzConstants.ACTION_COUNT_INDEX_PAO, 1);
...
//        JhPhzCheckCardBean checkCard = player.checkCard(0, isSelfMo(player), false, false, false, false, true);
//        checkPaohuziCheckCard(checkCard);
//
//        if (checkCard.isHu()) {
//            setHuCard(0);
...
//        }
		...
		// 是否能出牌
        if (!isHu) {
            sendActionMsg(player, action, cardList, JhPhzConstants.action_type_action, isZaiPao, !disCard, -1);
        } else {
            sendActionMsg(player, action, cardList, JhPhzConstants.action_type_action, isZaiPao, false, -1);
        }
...
//        }
	 ...
	 * @param action
	 */
    private void disCard(JhPhzPlayer player, List<Integer> cardList, int action) {
...
//			checkMo();
        ...
        JhPhzCheckCardBean autoDisCard = checkDisAction(player, action, cardList.get(0), true);
//		int canPaoSeat=0;
//        if (autoDisCard == null) {
//
//            for (Entry<Integer, JhPhzPlayer> entry : seatMap.entrySet()) {
//                if (entry.getKey().intValue() == player.getSeat()) {
//                    continue;
//                }
//                JhPhzCheckCardBean rets = entry.getValue().checkPaoHu(cardList.get(0), false, false);
//
//                if (rets.isPao()) {
//                    clearAction();
//                    break;
...
//
//        }
        JhPhzPlayer nexPlayer =	seatMap.get(calcNextSeat(player.getSeat()));
        int val = JhPhzCardUtils.loadCardVal(cardList.get(0));
   	 	checkSchi(player, nexPlayer, true, val);
        
        sendActionMsg(player, action, cardList, JhPhzConstants.action_type_dis);
        if (autoDisCard != null) {
			// 系统自动出牌
...
//                checkSendActionMsg();
	 ...
	 * @param action
	 */
    private void peng(JhPhzPlayer player, List<Integer> cardList, Integer nowDisCard, int action) {
        ...
        if (!checkAction(player, action)) {
            ...
            if (actionList.get(0).intValue() == 1) {
                LogUtil.monitorLog.info("can hu xuanze peng:tableId=" + getId() + ",userId=" + player.getUserId() + ",cardVal=" + JhPhzCardUtils.loadCardVal(nowDisCard) + ",card=" + nowDisCard);
...
//                addAction(player.getSeat(), actionList);
        ...
        int val = JhPhzCardUtils.loadCardVal(nowDisCard);
        cardList = JhPhzCardUtils.loadIdsByVal(player.getHandCards().INS, val);
	 ...
	 * @param action
	 */
    private void pass(JhPhzPlayer player, List<Integer> cardList, Integer nowDisCard, int action) {
        List<Integer> actionList = actionSeatMap.remove(player.getSeat());
        if (actionList == null) {
            return;
        }
        ...
        if (actionList.get(1).intValue() == 1) {
            int val = JhPhzCardUtils.loadCardVal(lastCard);
            player.passVal(action_peng, val);
            LogUtil.monitorLog.info("cc" + player.getUserId() + ",tableId=" + getId() + ",cardVal=" + val);
        }

        List<Integer> list = JhPhzConstants.parseToDisActionList(actionList, true);
        if (list.contains(JhPhzConstants.action_zai) || list.contains(JhPhzConstants.action_ti) || list.contains(JhPhzConstants.action_chouzai)) {
            return;
        }
        if (!list.contains(JhPhzConstants.action_chi) && !list.contains(JhPhzConstants.action_peng) && !list.contains(action_hu)
                && !list.contains(JhPhzConstants.action_wangdiao)
                && !list.contains(JhPhzConstants.action_wangchuang)
                && !list.contains(JhPhzConstants.action_wangzha)) {
            return;
        }

        boolean wangDiaoOrChuang = false;
        if (list.contains(JhPhzConstants.action_wangdiao) || list.contains(JhPhzConstants.action_wangchuang)
                || list.contains(JhPhzConstants.action_wangzha)) {
        ...
        if(action==JhPhzConstants.action_pass){
            ...
            addPlayLog(player.getSeat(), JhPhzConstants.action_guo + "",logId+"");
        ...
        int val = 0;
        if (nowDisCard != null) {
            val = JhPhzCardUtils.loadCardVal(nowDisCard);
        ...
        if (checkPao) {
            for (Entry<Integer, JhPhzPlayer> kv : seatMap.entrySet()) {
                JhPhzCheckCardBean rets = kv.getValue().checkPaoHu(nowDisCard, disCardSeat == kv.getKey().intValue(), false);
                if (rets.isPao()) {
                    ...
                    JhPhzCheckCardBean pccb = new JhPhzCheckCardBean();
                    pccb.setSeat(kv.getKey());
                    pccb.setDisCard(nowDisCard);
                    pccb.setPao(true);
                    pccb.setAuto(JhPhzConstants.action_pao, new ArrayList<>(list1));
        ...
        if (recallAction(player.getSeat(), action, Arrays.asList(nowDisCard == null ? 0 : nowDisCard, list.contains(JhPhzConstants.action_peng) ? 1 : 0, list.contains(JhPhzConstants.action_chi) ? 1 : 0))) {
            return;
        }
                    ...
                    for (Entry<Integer, String> kv : userActionMap.entrySet()) {
                        ...
                        if (strs.length == 4 && action_pass == Integer.parseInt(strs[0])) {
                            if (Integer.parseInt(strs[2]) == 1) {
                                int cardVal = JhPhzCardUtils.loadCardVal(Integer.parseInt(strs[1]));
...
//                    sendPlayerActionMsg(player, true);
                    sendPlayerActionMsg(player, true, cardList, JhPhzConstants.action_type_action, action);
                }

//				sendMoMsg(player, action, cardList, JhPhzConstants.action_type_action);
	 ...
	 * @param action
	 */
    private void chi(JhPhzPlayer player, List<Integer> cardList0, Integer nowDisCard, int action) {
    	 List<Integer> actionList = actionSeatMap.get(player.getSeat());
    	 if (actionList == null) {
             return;
         }
       ...
       // actionSeatMap.remove(player.getSeat());
        if (sendPaoSeat > 0) {
            ...
            LogUtil.monitorLog.info("can hu xuanze chi:tableId=" + getId() + ",userId=" + player.getUserId() + ",cardVal=" + JhPhzCardUtils.loadCardVal(nowDisCard) + ",card=" + nowDisCard);
        ...
        if (!checkAction(player, action)) {
            ...
            if (actionList.get(1).intValue() == 1 || actionList.get(0).intValue() == 1) {
                ...
                LogUtil.monitorLog.info("can hu or peng xuanze chi:tableId=" + getId() + ",userId=" + player.getUserId() + ",cardVal=" + JhPhzCardUtils.loadCardVal(nowDisCard) + ",card=" + nowDisCard);

                if (actionList.get(1).intValue() == 1) {
                    actionList.set(1, 0);
					// // 选择了吃，那不能碰了
                    player.passVal(JhPhzConstants.action_peng, JhPhzCardUtils.loadCardVal(nowDisCard));
                }
                if (actionList.get(0).intValue() == 1) {
                    actionList.set(0, 0);
                    ...
                    if (checkPao) {
                        for (Entry<Integer, JhPhzPlayer> kv : seatMap.entrySet()) {
                            JhPhzCheckCardBean rets = kv.getValue().checkPaoHu(nowDisCard, disCardSeat == kv.getKey().intValue(), false);
                            if (rets.isPao()) {
                                Integer val = JhPhzCardUtils.loadCardVal(nowDisCard);
                                ...
                                pccb.setDisCard(nowDisCard);
                                pccb.setPao(true);
                                pccb.setAuto(JhPhzConstants.action_pao, new ArrayList<>(list));
                                pccb.buildActionList();
                                checkPaohuziCheckCard(pccb);
                                setAutoDisBean(pccb);
        ...
        int val = JhPhzCardUtils.loadCardVal(nowDisCard);
        for (JhPhzPlayer player1 : playerMap.values()) {
	...
	private boolean checkChiXiaHuo(JhPhzPlayer player, List<Integer> cardList0, Integer nowDisCard,
...
//		}
    ...
    public synchronized void play(JhPhzPlayer player, List<Integer> cardIds, int action, boolean moPai, boolean isHu, boolean isPassHu) {
        if (state != SharedConstants.table_state.play) {
            return;
        }
        Integer nowDisCard = null;
        List<Integer> cardList = null;
        if (action != JhPhzConstants.action_mo) {
...
//		LogUtil.msgLog.info(player.getUserId()+",playerSeat:"+player.getSeat()+",moseat:"+moSeat+",currentSeat:"+disCardSeat+",next:"+nowDisCardSeat + ",action:" + action + ",nowDisCard:" +nowDisCard+ " -" + cardIds+",moPai:"+moPai+".isHu:"+isHu+",isPassHu:"+isPassHu);
        ...
        if (action == JhPhzConstants.action_ti) {
            if (cardList.size() > 4) {
        ...
        } else if (action == action_hu || action == JhPhzConstants.action_wangdiao || action == JhPhzConstants.action_wangchuang
                || action == JhPhzConstants.action_wangzha) {
            hu(player, cardList, action, nowDisCard);
        } else if (action == JhPhzConstants.action_peng) {
            peng(player, cardList, nowDisCard, action);
        } else if (action == JhPhzConstants.action_chi) {
            chi(player, cardList, nowDisCard, action);
        } else if (action == JhPhzConstants.action_pass) {
            pass(player, cardList, nowDisCard, action);
        } else if (action == JhPhzConstants.action_pao) {
            pao(player, cardList, nowDisCard, action, isHu, isPassHu);
        } else if (action == JhPhzConstants.action_zai || action == JhPhzConstants.action_chouzai) {
            zai(player, cardList, nowDisCard, action);
        } else if (action == JhPhzConstants.action_mo) {
//            if (checkMoMark != null) {
...
//            }
		...
		if (action >= JhPhzConstants.action_peng && action != JhPhzConstants.action_pass
				&& action != JhPhzConstants.action_mo) {
			sendTingInfo(player);
		}
	 ...
	 * @param player
	 */
    private boolean setDisPlayer(JhPhzPlayer player, int action, boolean isHu) {
	 ...
	 * @return
	 */
    public boolean checkAction(JhPhzPlayer player, int action) {
		// 优先度为胡杠补碰吃
        List<Integer> stopActionList = JhPhzConstants.findPriorityAction(action);
        for (Entry<Integer, List<Integer>> entry : actionSeatMap.entrySet()) {
            if (player.getSeat() != entry.getKey()) {
				// 别人
                boolean can = JhPhzConstants.canDis(stopActionList, entry.getValue(), disCardSeat == player.getSeat());
                if (!can) {
                    return false;
                }
                List<Integer> disActionList = JhPhzConstants.parseToDisActionList(entry.getValue(), disCardSeat == player.getSeat());
    ...
    private JhPhzPlayer getDisPlayer() {
    ...
    private void checkMo(boolean wangDiaoOrChuang) {
        synchronized (this) {
            ...
            int canHu = -1;
            JhPhzCardResult cardResult = null;
                    if (autoDisBean == null) {
                        if (leftCards == null) {
                            return;
                        }
                        ...
                        int tempCard = getNextLeftCard();
                            JhPhzPlayer player1 = seatMap.get(nowDisCardSeat);
                            if (player1.getHandCards().WANGS.size() > 0&&!wangDiaoOrChuang) {
                              //  int tempCard = getNextLeftCard();
                                JhPhzCheckCardBean checkCard1 = player1.checkCard(tempCard, true, false, false, false, false, true, 0);
                                if (checkCard1.isHu()) {
                                    canHu = 1;
                                    cardResult = checkCard1.getCardResult();
                                    if (checkCard1.isWangDiao() || checkCard1.isWangChuang() || checkCard1.isWangZha()) {
                                        addAction(player1.getSeat(), JhPhzCheckCardBean.hasActionList(checkCard1.isWangDiao() ? 7 : -1, checkCard1.isWangChuang() ? 8 : -1, checkCard1.isWangZha() ? 9 : -1));

                                        LogUtil.msgLog.info("tiqianhu:tableId=" + getId() + ",current=" + playBureau + ",userId=" + player1.getUserId() + ",seat=" + player1.getSeat() + ",cardVal=" + JhPhzCardUtils.loadCardVal(tempCard) + ",zha=" + checkCard1.isWangZha() + ",chuang=" + checkCard1.isWangChuang() + ",diao=" + checkCard1.isWangDiao());

                                        sendActionMsg(player1, JhPhzConstants.action_mo, Collections.<Integer>emptyList(), JhPhzConstants.action_type_mo, false, false, player1.getUserId());
...
//                }
			...
			// // 下一个要摸牌的人
            JhPhzPlayer player = seatMap.get(nowDisCardSeat);
            ...
            JhPhzCheckCardBean autoDisCard = null;
            addPlayLog(player.getSeat(), JhPhzConstants.action_mo + "", (card == null ? 0 : card) + "");
            logActionMsg(player, card, JhPhzConstants.action_mo, null);
...
//				player.moCard(card);
                    ...
                    cardList.add(card);
                    setNowDisCardIds(JhPhzCardUtils .asList(card));
                    player.setPassHuVal(0,0);
                    player.moWang(card);
                    JhPhzCheckCardBean checkCard = player.checkCard(card, true, canHu >= 0, false, false, false, canHu);
                    ...
                    sendActionMsg(player, JhPhzConstants.action_mowang, cardList, JhPhzConstants.action_type_action, false, false, -1);
...
//                        
//                        }
                       // sendActionMsg(player, JhPhzConstants.action_mo, new ArrayList<Integer>(), JhPhzConstants.action_type_mo);
                    } else {
                        if (toPlayCardFlag == 0) {
                            checkMo();
                        }
...
//				int huCount=0;
                Map<Integer, JhPhzCardResult> cardResultMap = new HashMap<>(8);

               Set<Integer> huList = new HashSet<>();
                for (Entry<Integer, JhPhzPlayer> entry : seatMap.entrySet()) {
                    boolean self = entry.getKey().intValue() == player.getSeat();
                    JhPhzCheckCardBean checkCard = entry.getValue().checkCard(card, self, false, false, false, false, canHu);
                     if (checkPaohuziCheckCard(checkCard)) {
                       autoDisCard = checkCard;
                       break;
//                       if(checkCard.isHu()) {
...
//						huCount++;
                        ...
                        if (self) {
                            if (cardResult != null) {
                                ...
                                cardResultMap.put(entry.getKey(), new JhPhzCardResult());
                            }
                ...
                if (autoDisCard != null) {
                    ...
                    for (Entry<Integer, JhPhzPlayer> entry : seatMap.entrySet()) {
                        boolean self = entry.getKey().intValue() == player.getSeat();
                        JhPhzCheckCardBean rets = entry.getValue().checkPaoHu(card, self, true);
                        List<Integer> actionList = actionSeatMap.get(entry.getKey());
                        if (actionList == null || actionList.isEmpty()) {
                            if (rets.isHu()) {
                              //  addAction(entry.getKey().intValue(), JhPhzCheckCardBean.uniqueHasActionList(5));
                            	  Integer val = JhPhzCardUtils.loadCardVal(card);
                            ...
                            if (rets.isHu()) {
                                setHuCard(card);
                                huList.add(entry.getKey());
                                JhPhzCardResult cr = cardResultMap.get(entry.getKey());
                                if (cr == null) {
                                    setPaoHu(entry.getKey());
                                } else {
                                    if (self && !cr.isCanHu()) {
                                        JhPhzCheckCardBean checkCard = entry.getValue().checkCard(card, self, false, false, false, false);
                                        if (checkCard.isHu()) {
                ...
                if(checkHuPlayer() && (autoDisCard==null ||autoDisCard.isPao())) {
					// 不是自摸不能自动胡
                		 ...
                		 for (Integer seat : huList) {
                			 JhPhzPlayer lPlayer = seatMap.get(seat);
                			 if(lPlayer!=null) {
                				 if((lPlayer.getHandCards().WANGS.size()==0) ||(!isMoFlag())) {
                					 
                				 if(checkAction(lPlayer, JhPhzConstants.action_hu)) {
                					 JhPhzCheckCardBean pccb = new JhPhzCheckCardBean();
                					 pccb.setSeat(seat);
                						pccb.setDisCard(disposeCard);
                						pccb.setAuto(JhPhzConstants.action_hu, new ArrayList<>());
                						pccb.buildActionList();
                						checkPaohuziCheckCard(pccb);
                						setAutoDisBean(pccb);
...
//                					 actionSeatMap.remove(key)
                					 ...
                					 actionList =  JhPhzCardUtils.keepHu(actionList);
                ...
                JhPhzPlayer nextPlayer = seatMap.get(calcNextSeat(player.getSeat()));
                int val = JhPhzCardUtils.loadCardVal(card);
                checkSchi(player, nextPlayer, true, val);
                

                markMoSeat(player.getSeat(), JhPhzConstants.action_mo);
                if (autoDisCard != null && (autoDisCard.getAutoAction() == JhPhzConstants.action_zai || autoDisCard.getAutoAction() == JhPhzConstants.action_chouzai || autoDisCard.getAutoAction() == JhPhzConstants.action_ti)) {
                    sendMoMsg(player, JhPhzConstants.action_mo, new ArrayList<>(Arrays.asList(card)), JhPhzConstants.action_type_mo);
                } else {
                    sendActionMsg(player, JhPhzConstants.action_mo, new ArrayList<>(Arrays.asList(card)), JhPhzConstants.action_type_mo);
                }
    ...
    public void logActionMsg(JhPhzPlayer player, int id,int action, List<Integer> actList) {
    	try {
            ...
            sb.append("|").append(JhPhzConstants.renameAction(action));
    ...
    private void checkSchi(JhPhzPlayer cPlayer,JhPhzPlayer nextPlayer,boolean isMo,int cardVal){
	...
	private void setPaoCard(Entry<Integer, JhPhzPlayer> entry, List<Integer> list, JhPhzCheckCardBean pccb) {
	 ...
	 * @return 是否有系统帮助自动出牌
	 */
    private JhPhzCheckCardBean checkDisAction(JhPhzPlayer player, int action, Integer disCard, boolean isPassHu) {
        JhPhzCheckCardBean result = null;
        for (Entry<Integer, JhPhzPlayer> entry : seatMap.entrySet()) {
            if (entry.getKey().intValue() == player.getSeat()) {
                continue;
            }

            JhPhzCheckCardBean checkCard = entry.getValue().checkCard(disCard, false, isPassHu, false, false, false);
            ...
            if (!checkCard.isHu()) {
                JhPhzCheckCardBean rets = entry.getValue().checkPaoHu(disCard, entry.getKey().intValue() == player.getSeat(), true);
                ...
                if(rets.isHu()) {
                	...
                	result = biHuAuto(result, entry, checkCard);
                         ...
                         Integer val = JhPhzCardUtils.loadCardVal(disCard);
                         List<Integer> list = entry.getValue().getHandCards().KAN.get(val);
                         if (list == null) {
                             list = entry.getValue().getHandCards().WEI.get(val);
                             if (list == null) {
...
//                    if(huBiHu>=1) {
//                    	rets.setAuto(JhPhzConstants.action_hu, new ArrayList<>());
//                    	setPaoHu(entry.getKey());
//                     	setAutoDisBean(rets);
//                     	result = rets;
//                     }
	...
	private JhPhzCheckCardBean biHuAuto(JhPhzCheckCardBean result, Entry<Integer, JhPhzPlayer> entry,
			JhPhzCheckCardBean checkCard) {
//		if(huBiHu>=1) {
			 checkCard.setAuto(JhPhzConstants.action_hu, new ArrayList<>());
			 setPaoHu(entry.getKey());
		 	setAutoDisBean(checkCard);
...
//		 }
    ...
    public boolean checkPaohuziCheckCard(JhPhzCheckCardBean checkCard) {
    ...
    public void setAutoDisBean(JhPhzCheckCardBean autoDisBean) {
    ...
    private void addAction(int seat, List<Integer> actionList) {
        actionSeatMap.put(seat, actionList);
        addPlayLog(seat, JhPhzConstants.action_hasaction + "", StringUtil.implode(actionList));
        saveActionSeatMap();
    }
    ...
    private void sendActionMsg(JhPhzPlayer player, int action, List<Integer> cards, int actType) {
	 ...
	 * @param actType
	 */
    private void sendMoMsg(JhPhzPlayer player, int action, List<Integer> cards, int actType) {
    ...
    private void sendPlayerActionMsg(JhPhzPlayer player, boolean force) {
        sendPlayerActionMsg(player, force, null, 0, JhPhzConstants.action_refreshaction);
    }
	 ...
	 * @param player
	 */
    private void sendPlayerActionMsg(JhPhzPlayer player, boolean force, List<Integer> cards, int actType, int action) {
	 ...
	 * @param actType
	 */
    private void sendActionMsg(JhPhzPlayer player, int action, List<Integer> cards, int actType, boolean isZaiPao, boolean isChongPao, long userId) {
        PlayPaohuziRes.Builder builder = PlayPaohuziRes.newBuilder();
        builder.setAction(action);
        builder.setUserId(player.getUserId() + "");
        builder.setSeat(player.getSeat());
...
//            }
    ...
    private void sendMsgBySelfAction(PlayPaohuziRes.Builder builder, long userId) {
        ...
        // boolean hasHu = false;
        int paoSeat = 0;
        if (JhPhzConstants.action_type_dis == actType || JhPhzConstants.action_type_mo == actType) {
            for (Entry<Integer, List<Integer>> entry : actionSeatMap.entrySet()) {
...
//                if (JhPhzConstants.action_type_dis == actType && player.isAutoPlay()) {
//                    copy.setActType(JhPhzConstants.action_type_dis_auto);
//                }
//            } else
			...
			// 需要特殊处理一下栽
            if (copy.getAction() == JhPhzConstants.action_zai || copy.getAction() == JhPhzConstants.action_chouzai || copy.getAction() == JhPhzConstants.action_ti) {
...
//					if(copy.getAction() == JhPhzConstants.action_chouzai){
...
//					copy.addAllPhzIds(ids);
    ...
    private void checkSendActionMsg() {
        ...
        JhPhzPlayer disCSMajiangPlayer = seatMap.get(disCardSeat);
        ...
        for (Entry<Integer, List<Integer>> entry : actionSeatMap.entrySet()) {
            ...
            copy.addAllSelfAct(actionList);
            JhPhzPlayer seatPlayer = seatMap.get(entry.getKey());
    ...
    public void checkAction() {
		...
		// 先判断拿牌的玩家
        JhPhzPlayer nowPlayer = seatMap.get(nowSeat);
        if (nowPlayer == null) {
            return;
        }
        JhPhzCheckCardBean checkCard = nowPlayer.checkCard(null, true, false, false, false, false);
	 ...
	 * @param checkCard
	 */
    public void playAutoDisCard(JhPhzCheckCardBean checkCard) {
	 ...
	 *            是否是摸牌 如果是摸牌，需要
	 */
    public void playAutoDisCard(JhPhzCheckCardBean checkCard, boolean moPai) {
...
//        }
    ...
    protected synchronized void deal() {
        ...
        if (lastWinSeat == 0 && seatMap.size() > 0) {
            JhPhzPlayer player = playerMap.get(masterId);
...
//        }
        ...
        if(zp != null && !zp.isEmpty()) {
        	list = JhPhzCardUtils.fapai2(JhPhzCardUtils.loadCards(wangbaCount > 0 ? wangbaCount : 0), zp);
        }else {
//        	list = JhPhzCardUtils.loadCards(JhPhzCardUtils.loadCards(wangbaCount > 0 ? wangbaCount : 0),  maxPlayerCount, 14);
            list = JhPhzCardUtils.fapaiControl(JhPhzCardUtils.loadCards(wangbaCount > 0 ? wangbaCount : 0), maxPlayerCount, 14,0);
        }
        //maxPlayerCount >= 3 ? 3 :
        int i = 1;
        for (JhPhzPlayer player : playerMap.values()) {
...
//        }
    ...
    public CreateTableRes buildCreateTableRes(long userId, boolean isrecover, boolean isLastReady) {
        ...
        if (lastSeatAction != null) {
            if (JhPhzConstants.action_ti == lastSeatAction.getValue() || JhPhzConstants.action_zai == lastSeatAction.getValue() || JhPhzConstants.action_chouzai == lastSeatAction.getValue()) {
                tiOrZai = true;
            }
        }
...
//    public int getBopiPoint(JhPhzPlayer player) {
//        if (!isBoPi()) {
//            return 0;
//        }
//
...
//        int otherPoint = 0;
//        int retPoint = 0;
//        for (JhPhzPlayer temp : seatMap.values()) {
...
//            list.add(build.build());
        ...
        if (isCreditTable()) {
            ...
            for (ClosingPhzPlayerInfoRes.Builder builder : builderList) {
            	JhPhzPlayer player = seatMap.get(builder.getSeat());
        ...
        if (winPlayer != null) {
            ...
            replaceCards = winPlayer.buildReplaceCards(JhPhzCardUtils.loadCardVal(fanXinCardId));
...
//                    }
    ...
    public void sendTingInfo(JhPhzPlayer player) {
        if(player.isAutoPlay()){
            return;
        }
        long start = System.currentTimeMillis();
        int size = player.getHandCards().INS.size() + player.getHandCards().WANGS.size();
        JhPhzHandCards cards = new JhPhzHandCards();
...
//        Collection<Integer> huCardList = new ArrayList<>(GuihzCard.huCardList);
        ...
        if (size % 3 == 0 || (size2 > 0 && size % 3 == 2)) {
            ...
            for (int id : player.getHandCards().INS) {
                ...
                for (Integer id2 : huCardList) {
                    ...
                    JhPhzCardResult cardResult = JhPhzHuPaiUtils.huPai(cards, id2, this, true, player.getUserId(), loadXingCard(false));
                    if (cardResult.isCanHu()) {
                        map.put(card.getVal(), id2);
                    }else{
                    	  JhPhzCheckCardBean rets = player.checkPaoHu2(cards, id2, true, true);
                          if (rets.isHu()) {
                              map.put(card.getVal(), id2);
                          }
                    }
            ...
            for (Integer id : huCardList) {
                GuihzCard card = GuihzCard.getPaohzCard(id);
                JhPhzCardResult cardResult = JhPhzHuPaiUtils.huPai(cards, id, this, true, player.getUserId(), loadXingCard(false));
                if (cardResult.isCanHu()) {
                    map.put(card.getVal(), id);
                }
                JhPhzCheckCardBean rets = player.checkPaoHu2(cards, id, true, true);
                if (rets.isHu()) {
                    map.put(card.getVal(), id);
                }
            }
    ...
    public Collection<Integer> getHuCards(JhPhzPlayer player){
...
//         List<String> strParams = createTableInfo.getStrParams();
		...
		autoPlay = tuoguan>0;
		 ...
		 shouJuZhuang = StringUtil.getIntValue(params, 16, 0);
          this.hongHeiDian = StringUtil.getIntValue(params, 17, 0);
		// 加倍：0否，1是
...
//        }
    ...
    public void checkReconnect(Player player) {
        ...
        sendTingInfo((JhPhzPlayer) player);
    }
...
//	                return;
                ...
                if (actionSeatMap.size() > 0) {
                	   int action = 0,seat = 0;
                       for (Entry<Integer, List<Integer>> entry : actionSeatMap.entrySet()){
                           List<Integer> list = JhPhzConstants.parseToDisActionList(entry.getValue(),false);
...
//                            }
                        ...
                        if (auto || player.isRobot()) {
                            ...
                            } else if (player.getAutoPlayTime() > 0L && currentTime - player.getAutoPlayTime() >= JhPhzConstants.AUTO_PLAY_TIME) {
...
//                            }
                        ...
                        if (auto || player.isRobot()) {
                            if (player.getAutoPlayTime() == 0L) {
                                ...
                                for (int i = 0, len = cards0.size() - 1; i < len; i++) {
                                    if (JhPhzCardUtils.loadCardVal(cards0.get(i)) != JhPhzCardUtils.loadCardVal(cards0.get(i + 1))) {
                                        card = cards0.get(i);
                                        break;
                                    } else {
                                        i++;
    ...
    public int getHongHeiDian() {
        return hongHeiDian;
    }

    public void setHongHeiDian(int hongHeiDian) {
        this.hongHeiDian = hongHeiDian;
    }
