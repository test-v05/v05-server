...
import com.sy599.game.qipai.jhphz281.command.JhPhzCommandProcessor;
import com.sy599.game.qipai.jhphz281.util.*;
...
public class JhPhzPlayer extends Player {
    // 座位id
    private int seat;
    // 状态
    ...
    private List<JhPhzCardTypeHuxi> cardTypes;

    public List<JhPhzCardTypeHuxi> getCardTypes() {
        return cardTypes;
    }
    ...
    private JhPhzHandCards handCards = new JhPhzHandCards();
    private JhPhzCardResult huResult;
...
//        }
        ...
        if (this.autoPlay != autoPlay){
			...
			for (Map.Entry<Long,Player> kv:table.getPlayerMap().entrySet()){
       		 ...
       		 table.addPlayLog(getSeat(), JhPhzConstants.action_tuoguan + "",(autoPlay?1:0) + "");
		}
		this.autoPlay = autoPlay;
    ...
    public JhPhzHandCards getHandCards() {
        return handCards;
    }
...
//				int val=JhPhzCardUtils.loadCardVal(disCard);
...
//		}
        ...
        if (disCardList.size() > 1) {
            ...
            Iterator<JhPhzCardTypeHuxi> iterator = cardTypes.iterator();
            while (iterator.hasNext()) {
                JhPhzCardTypeHuxi type = iterator.next();
                if (type.isHasCard(card)) {
                    if (type.getAction() == JhPhzConstants.action_zai) {
        ...
        if (action == JhPhzConstants.action_ti) {
            ...
            if (copy.size() % 3 == 0) {
                for (int i = 0; i < copy.size() / 3; i++) {
                    ...
                    if (JhPhzCardUtils.loadCardCount(list).size() == 3) {
    ...
    public void addCardType(int action, List<Integer> disCardList) {
        int huxi = JhPhzCardUtils.countHuxi0(new ArrayList<>(disCardList));
        if (action != 0) {
            JhPhzCardTypeHuxi type = new JhPhzCardTypeHuxi();
            type.setAction(action);
            type.setCardIds(disCardList);
            type.setHux(huxi);
            cardTypes.add(type);

            if (action == JhPhzConstants.action_zai || action == JhPhzConstants.action_chouzai || action == JhPhzConstants.action_ti) {
                changeZaihu(huxi);
            } else {
    ...
    public void passVal(int action, int val) {
        if (JhPhzCardUtils.commonCardByVal(val)){
            if (action == JhPhzConstants.action_chi) {
                handCards.NOT_CHI.add(val);
                changeExtend();
                changeTableInfo();
            } else if (action == JhPhzConstants.action_peng) {
...
//    public JhPhzCheckCardBean checkTi() {
//        JhPhzCheckCardBean check = new JhPhzCheckCardBean();
//        check.setSeat(seat);
...
////                list.addAll(temp);
////            }
////            check.setAuto(JhPhzConstants.action_ti, list);
////            check.setTi(true);
//        }
...
//    }
    ...
    public List<Integer> getCalcOverCards() {
        ...
        if (huResult != null && huResult.isCanHu()) {
            for (JhPhzCardMessage temp : huResult.getCardMessageList()) {
    ...
    public boolean isCanDisCard(List<Integer> disCards, Integer nowDisCard, int action) {
        if (disCards != null) {
            if (action == JhPhzConstants.action_zai || action == JhPhzConstants.action_chouzai) {
                return disCards.size() == 2 && handCards.INS.containsAll(disCards);
            } else if (action == JhPhzConstants.action_peng) {
                int val = JhPhzCardUtils.loadCardVal(nowDisCard);
                return JhPhzCardUtils.loadIdsByVal(handCards.INS, val).size() == 2;
            } else if (action == JhPhzConstants.action_ti) {
                int val = JhPhzCardUtils.loadCardVal(nowDisCard);
...
//		if (card != null) {
//
//			JhPhzTable table = getPlayingTable(JhPhzTable.class);
...
//			if (sameList.size() >= 3) {
//				if (isSelfMo) {
//					action = JhPhzConstants.action_ti;
//				} else {
//					action = JhPhzConstants.action_pao;
//				}
//			}
//
...
//	public JhPhzCheckCardBean checkPaoHu() {
//		JhPhzCheckCardBean check = new JhPhzCheckCardBean();
//		check.setSeat(seat);
//
//		JhPhzTable table = getPlayingTable(JhPhzTable.class);
...
//	}
     ...
     * @return
     */
    public JhPhzCheckCardBean checkPaoHu(Integer card0, boolean isSelfMo, boolean checkHu) {
        JhPhzCheckCardBean paohuziCheckCardBean = new JhPhzCheckCardBean();
        paohuziCheckCardBean.setSeat(seat);

        int card = card0 == null ? 0 : card0.intValue();
        if (JhPhzCardUtils.commonCard(card)) {
            List<Integer> list;
...
//                    result[0] = true;
                    ...
                    if (canHuState == 1 && checkHu) {
                        JhPhzCardResult cardResult = JhPhzHuPaiUtils.huPai(handCards1, 0, table, isSelfMo, userId, table.loadXingCard(false));
...
//                                    result[1] = true;
    ...
    public JhPhzCheckCardBean checkPaoHu2(JhPhzHandCards mHandCards,Integer card0, boolean isSelfMo, boolean checkHu) {
        JhPhzCheckCardBean paohuziCheckCardBean = new JhPhzCheckCardBean();
        paohuziCheckCardBean.setSeat(seat);
...
//                     result[0] = true;
                     paohuziCheckCardBean.setPao(true);
                     JhPhzHandCards handCards1 = mHandCards.copy();
                     list = handCards1.KAN.remove(val);
                     list.add(card);
                     handCards1.PAO.put(val, list);
                     if (canHuState == 1 && checkHu) {
...
//                                      result[1] = true;
    ...
    public JhPhzCheckCardBean checkCard(Integer card, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass) {
        return checkCard(card, isSelfMo, isPassHu, isBegin, isFirstCard, isPass, false, 0, false, -1);
    }

    public JhPhzCheckCardBean checkCard(Integer card0, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass, int huResult) {
        return checkCard(card0, isSelfMo, isPassHu, isBegin, isFirstCard, isPass, false, 0, false, huResult);
    }

    public JhPhzCheckCardBean checkCard(Integer card0, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass, boolean wangPass) {
        return checkCard(card0, isSelfMo, isPassHu, isBegin, isFirstCard, isPass, false, 0, wangPass, -1);
    }

    public JhPhzCheckCardBean checkCard(Integer card0, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass, boolean tiqian, Integer preCard) {
        return checkCard(card0, isSelfMo, isPassHu, isBegin, isFirstCard, isPass, tiqian, preCard, false, -1);
    }

    private int replaceRedCount(JhPhzCardResult cardResult) {
        int count = 0;
        for (String str : cardResult.getReplaceCards()) {
            String[] strs = str.split("\\|");
            for (String s : strs) {
                if (s.length() > 0) {
                    if (JhPhzCardUtils.redCard(Integer.parseInt(s))) {
                        count++;
                        break;
                    }
                }
    ...
    private boolean replaceRed(JhPhzCardResult cardResult) {
     ...
     * @return
     */
    public JhPhzCheckCardBean checkCard(Integer card0, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass, boolean tiqian, Integer preCard, boolean wangPass, int huResult) {
        int card = card0 == null ? 0 : card0.intValue();
        JhPhzCheckCardBean check = new JhPhzCheckCardBean();
        check.setSeat(seat);
        ...
        if (tiqian) {
            if (canHuState == 1) {
//				if (preCard!=null&&JhPhzCardUtils.commonCard(preCard)){
//					int val=JhPhzCardUtils.loadCardVal(preCard);
//					if(nextSeat==seat){
//						if (val<200&&!isPass&&canChi0(preCard)&&handCards.INS.size()>2){
//							if (isMoFlag){
//								check.setChi(true);
//							}else{
//								List<Integer> list=new ArrayList<>(((JhPhzPlayer)table.getSeatMap().get(table.calcFrontSeat(seat))).getHandCards().OUTS);
//                                list.remove(Integer.valueOf(table.getDisposeCard()));
//								boolean bl=true;
//								for (Integer integer:list){
//								    if (val==JhPhzCardUtils.loadCardVal(integer)){
//                                        bl=false;
//                                        break;
//                                    }
//                                }
...
//					}
//
//					if (!isPass&&!handCards.NOT_PENG.contains(val)&&JhPhzCardUtils.canPeng(handCards.INS,preCard)&&handCards.INS.size()>2){
//						check.setPeng(true);
//					}
//				}

                JhPhzCardResult cardResult = JhPhzHuPaiUtils.huPai(handCards, card, table, isSelfMo, userId, table.loadXingCard(tiqian));
                if (cardResult.isCanHu()) {
                    check.setHu(true);
                    check.setCardResult(cardResult);
                    boolean checkHu = true;
                    if ((card > 0 && !JhPhzCardUtils.smallCard(card)) && (cardResult.isWangZha() || cardResult.isWangChuang())) {
                        int huxi = cardResult.getTotalHuxi(false);
                        if (cardResult.isWangZha()) {
                            JhPhzCardMessage cardMessage = null;
                            for (JhPhzCardMessage cm : cardResult.getCardMessageList()) {
                                if (cm.getHuXiEnum() == JhPhzHuXiEnums.TI && JhPhzCardUtils.hasCard(cm.getCards(), val) && JhPhzCardUtils.countCard(cm.getCards(), 201) >= 3) {
                                    cardMessage = cm;
                                    break;
                                }
                            }
                            ...
                            if (cardMessage != null && huxi - cardMessage.loadHuxi() + 9 < table.loadQihuxi()) {
                                ...
                                if ((card > 0 && !JhPhzCardUtils.smallCard(card)) && cardResult.isWangChuang()) {
                                    huxi = cardResult.getChuangCardResult().getTotalHuxi(false);
                                    cardMessage = null;
                                    for (JhPhzCardMessage cm : cardResult.getChuangCardResult().getCardMessageList()) {
                                        if ((cm.getHuXiEnum() == JhPhzHuXiEnums.WEI || cm.getHuXiEnum() == JhPhzHuXiEnums.KAN) && JhPhzCardUtils.hasCard(cm.getCards(), val) && JhPhzCardUtils.countCard(cm.getCards(), 201) >= 2) {
                                            cardMessage = cm;
                                            break;
                                        }
                                    }
                                    ...
                                    if (cardMessage != null && huxi - cardMessage.loadHuxi() + 3 < table.loadQihuxi()) {
                                        ...
                                        if (handCards.WANGS.size() >= 4 && table.loadHuMode() == 2 && !JhPhzCardUtils.wangCard(card) && cardResult.getChuangCardResult().getTotalFan() > 1) {
                                            ...
                                            int red1 = JhPhzCardUtils.redCard(card) ? red0 - 2 : red0;
                                    ...
                                    if (cardResult.isWangChuang() && handCards.WANGS.size() >= 4 && table.loadHuMode() == 2 && !JhPhzCardUtils.wangCard(card) && cardResult.getChuangCardResult().getTotalFan() > 1) {
                                        int redCount = cardResult.getChuangCardResult().getRedTotal();
                                        int baseCount = cardResult.getChuangCardResult().getRedCount();
                                        int red0 = replaceRedCount(cardResult.getChuangCardResult());
                                        int red1 = JhPhzCardUtils.redCard(card) ? red0 - 2 : red0;
                                ...
                                if (cardResult.isWangDiao() && table.loadHuMode() == 2 && cardResult.getDiaoCardResult().getTotalFan() > 1) {
                                    ...
                                    if (JhPhzCardUtils.wangCard(card)) {
                                        if (red0 >= 3) {
                                            red1 = red0 - 2;
                                        } else {
                                        ...
                                        red1 = JhPhzCardUtils.redCard(card) ? red0 - 1 : red0;
                                    }
                    ...
                    if (checkHu) {
                        ...
                        if (cardResult.isWangZha()) {
                            ...
                            if (check.isWangChuang()) {
                                if (handCards.WANGS.size() >= 4 && !JhPhzCardUtils.wangCard(card) && table.loadHuMode() == 2 && cardResult.getChuangCardResult().getTotalFan() > 1) {
                                    int redCount = cardResult.getChuangCardResult().getRedTotal();
                                    int baseCount = cardResult.getChuangCardResult().getRedCount();
                                    int red0 = replaceRedCount(cardResult.getChuangCardResult());
                                    int red1 = JhPhzCardUtils.redCard(card) ? red0 - 2 : red0;
                ...
                if (isSelfMo) {
                    if (isMoFlag) {
                        ...
                        if (temp != null&&!temp.contains(card)) {
                            check.setTi(true);
                            check.setAuto(JhPhzConstants.action_ti, new ArrayList<>(temp));
                        } else {
                            ...
                            if (temp != null&&!temp.contains(card)) {
                                ...
                                if (temp != null&&!temp.contains(card)) {
//                                    check.setPao(true);
//                                    check.setAuto(JhPhzConstants.action_pao,new ArrayList<>(temp));
//
                                    ...
                                    temp = JhPhzCardUtils.loadIdsByVal(handCards.INS, val);
                                    if (temp != null && temp.size() == 2&&!temp.contains(card)) {
                                        if (!handCards.NOT_PENG.contains(val) && !handCards.NOT_CHI.contains(val)) {
                                            check.setZai(true);
                                            check.setAuto(JhPhzConstants.action_zai, new ArrayList<>(temp));
                                        } else {
                                            check.setChouZai(true);
                                            check.setAuto(JhPhzConstants.action_chouzai, new ArrayList<>(temp));
                                        }
                                    } else {
...
//
                                ...
                                if (!isPass && !handCards.NOT_PENG.contains(val) && JhPhzCardUtils.canPeng(handCards.INS, card) && handCards.INS.size() > 2) {
                                    check.setPeng(true);
                                }
...
//				if (!isPassHu&&isMoFlag){
//					JhPhzCardResult cardResult=JhPhzHuPaiUtils.huPai(handCards,card,15,isSelfMo);
//					if (cardResult.isCanHu()){
//						if(nextSeat==seat){
//							if (cardResult.isWangZha()){
...
//				check.setAuto(JhPhzConstants.action_hu, new ArrayList<>());
//			}
		}
...
//		LogUtil.msgLog.info("dongzuo list===tableId="+table.getId()+",userId="+userId+",actions="+list);
    ...
    public String buildOutPaiStr() {
        ...
        strBuilder.setLength(0);
        for (JhPhzCardTypeHuxi huxi : cardTypes) {
...
//		this.loseCnt=lostCount;
    ...
    public PlayerInTableRes.Builder buildPlayInTableInfo() {
        return buildPlayInTableInfo(getPlayingTable(JhPhzTable.class),0, false);
    }

    public PlayerInTableRes.Builder buildPlayInTableInfo(JhPhzTable table, long lookUid, boolean isrecover) {
...
//		}
    ...
    public List<Integer> loadFan(int hongHeiDian) {
        List<Integer> list = new ArrayList<>();
        if (huResult != null && huResult.isCanHu()) {
            for (JhPhzFanEnums fanEnums : huResult.getFans()) {
                switch (fanEnums) {
                    case WANG_DIAO:
                        list.add(1);
                        break;
    ...
    public int getTotalFan() {
        if (huResult != null && huResult.isCanHu()) {
           return JhPhzHuPaiUtils.loadMaxFanCount(huResult.getFans());
        }
        return 1;
    }
    public int getTotalFan(int hongHeiDian) {
        if (huResult != null && huResult.isCanHu()) {
            return JhPhzHuPaiUtils.loadMaxFanCount2(huResult.getFans(),hongHeiDian);
        }
        return 1;
    }
                ...
                if (StringUtils.isNotBlank(tis)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.TI.put(JhPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
                ...
                if (StringUtils.isNotBlank(paos)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.PAO.put(JhPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
                ...
                if (StringUtils.isNotBlank(kans)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.KAN.put(JhPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
                ...
                if (StringUtils.isNotBlank(weis)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.WEI.put(JhPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
                ...
                if (StringUtils.isNotBlank(weichous)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.WEI_CHOU.put(JhPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
                ...
                if (StringUtils.isNotBlank(pengs)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.PENG.put(JhPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
    ...
    public void calcResult(int count, int point, boolean huangzhuang) {
    	...
    	JhPhzTable table2 = getPlayingTable(JhPhzTable.class);
        if (!huangzhuang) {
            if (point > 0) {
                this.winCount += count;
            } else {
...
//			for(Map.Entry<Integer,List<Integer>> kv:handCards.TI.entrySet()){
//				total+=(kv.getKey()>100? JhPhzHuXiEnums.TI.getBig(): JhPhzHuXiEnums.TI.getSmall());
//			}
//		}
//		if (handCards.KAN.size()>0){
//			for(Map.Entry<Integer,List<Integer>> kv:handCards.KAN.entrySet()){
//				total+=(kv.getKey()>100? JhPhzHuXiEnums.KAN.getBig(): JhPhzHuXiEnums.KAN.getSmall());
//			}
//		}

        if (cardTypes.size() > 0) {
            for (JhPhzCardTypeHuxi cth : cardTypes) {
                if (cth.getAction() == 4) {
                    total += cth.getHux();
                }
            }
        ...
        if (handCards.WEI.size() > 0) {
            for (Map.Entry<Integer, List<Integer>> kv : handCards.WEI.entrySet()) {
                total += (kv.getKey() > 100 ? JhPhzHuXiEnums.WEI.getBig() : JhPhzHuXiEnums.WEI.getSmall());
            }
        }
        return total;
    }
    ...
    public List<Integer> loadHandCards() {
        ...
        for (JhPhzCardTypeHuxi cth : cardTypes) {
            if (cth.getAction() == JhPhzConstants.action_ti) {
    ...
    public JhPhzCardTypeHuxi convertCardType(List<Integer> list) {
        JhPhzCardTypeHuxi cth = new JhPhzCardTypeHuxi();
        JhPhzCardUtils.sort(list);
        cth.setCardIds(list);
        if (list.size() == 3) {
            int val1 = JhPhzCardUtils.loadCardVal(list.get(0));
            int val2 = JhPhzCardUtils.loadCardVal(list.get(1));
            int val3 = JhPhzCardUtils.loadCardVal(list.get(2));
            List<Integer> list1 = JhPhzCardUtils.loadIdsByVal(list, 201);
            if (list1.size() == 0) {
                if (val1 == val2 && val1 == val3) {
                    cth.setAction(JhPhzConstants.action_zai);
                    cth.setHux(val1 > 100 ? 6 : 3);
                } else if (val1 == 1 && val2 == 2 && val3 == 3) {
                    cth.setAction(JhPhzConstants.action_chi);
                    cth.setHux(3);
                ...
                if (val1 == 201) {
                    ...
                    if (val1 == val2) {
                        cth.setAction(JhPhzConstants.action_zai);
                        cth.setHux(val1 > 100 ? 6 : 3);
                    } else if (JhPhzCardUtils.sameType(list.get(0), list.get(1))) {
                        cth.setAction(JhPhzConstants.action_chi);
                        if (val1 == 1 || (val1 == 2 && val2 == 3)) {
                            cth.setHux(3);
                        } else if (val1 == 101 || (val1 == 102 && val2 == 103)) {
                            cth.setHux(6);
                        } else if (JhPhzCardUtils.redCard(list.get(0)) && JhPhzCardUtils.redCard(list.get(1))) {
                            cth.setHux(val1 > 100 ? 6 : 3);
                        } else {
                            cth.setHux(0);
                        }
            ...
            cth.setAction(JhPhzConstants.action_ti);
            cth.setHux(JhPhzCardUtils.loadCardVal(list.get(0)) > 100 ? 12 : 9);
        }
        return cth;
    }

    public int loadWangHu(JhPhzCardResult cardResult) {
        JhPhzCardResult temp = cardResult != null ? cardResult : huResult;
        int wangHu = -1;
        if (temp.isCanHu()) {
            wangHu = 0;
            for (JhPhzFanEnums fanEnums : temp.getFans()) {
    ...
    public int loadValCount(JhPhzCardResult cardResult, int val) {
        JhPhzCardResult temp = cardResult != null ? cardResult : huResult;
    ...
    public List<PhzHuCards> buildPhzHuCards() {
        ...
        if (huResult != null && huResult.isCanHu()) {
            for (JhPhzCardMessage cm : huResult.getCardMessageList()) {
...
//			System.out.println(huResult.getCardMessageList());
    ...
    public List<PhzHuCards> buildPhzCards(long lookUid) {
        List<PhzHuCards> list = new ArrayList<>();
        for (JhPhzCardTypeHuxi type : cardTypes) {
            if (lookUid != this.userId && type.getAction() == JhPhzConstants.action_zai) {
                // 不是本人并且是栽
     ...
     * @return
     */
    public int calcHuPoint(JhPhzTable table) {
    ...
    public JhPhzCardResult getHuResult() {
        return huResult;
    }

    public void setHuResult(JhPhzCardResult huResult) {
...
//        int val = JhPhzCardUtils.loadCardVal(id);
//        return !JhPhzCardUtils.hasCard(handCards.OUTS, val) && !handCards.NOT_CHI.contains(val) && JhPhzCardUtils.canChi(handCards.INS, id).size() > 0;
//    }
    ...
    public boolean canChi0(int id) {
        ...
        JhPhzTable table = getPlayingTable(JhPhzTable.class);
        if (table != null) {
            JhPhzPlayer player = (JhPhzPlayer) table.getSeatMap().get(table.calcFrontSeat(seat));
            Set<Integer> set = new HashSet<>(player.getHandCards().OUTS);
            set.remove(Integer.valueOf(id));
            if (JhPhzCardUtils.hasCard(set, val)) {
                return false;
            }
        }
...
//    }
    ...
    public static void loadWanfaPlayers(Class<? extends Player> cls){
        for (Integer integer:wanfaList){
            PlayerManager.wanfaPlayerTypesPut(integer,cls, JhPhzCommandProcessor.getInstance());
        }
    }
    public Iterable<? extends PhzHuCards> buildNormalPhzHuCards() {
        List<PhzHuCards> list = new ArrayList<>();
        if(null!= huResult && huResult.isCanHu() ){
            for (JhPhzCardMessage msg:huResult.getCardMessageList()
            ) {
                int _hx =0;
                if( msg.getCards().get(0)<=40){
                    _hx = msg.getHuXiEnum().getSmall();
                }else{
                    _hx = msg.getHuXiEnum().getBig();
                }
                list.add(new JhPhzCardTypeHuxi(  msg.getHuXiEnum().getAction(),msg.getCards(), _hx).buildMsg().build());
            }
        }else{
            for (JhPhzCardTypeHuxi type : cardTypes) {
                //修正错误h胡息数
                int isbig =0;
                if( type.getCardIds().get(0)<=40){
                    isbig =1;
                }else{
                    isbig =2;
                }
                int _hx =0;
                //peng action=2  da=3 xiao =1;
                //pao action=7 da=9 xiao = 6
            ...
            List<Integer> _hand =  new ArrayList<>(handCards.INS);
            _hand.addAll(handCards.WANGS);
            msg.addAllCards(_hand) ;
            msg.setAction(0);
            msg.setHuxi(0);
            list.add(msg.build());
        }
