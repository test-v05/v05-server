...
import com.sy599.game.qipai.yzphz.command.YzPhzCommandProcessor;
...
import com.sy599.game.qipai.yzphz.util.YzPhzHuXiEnums;
...
public class YzPhzPlayer extends Player {
    // 座位id
    private int seat;
    // 状态
    ...
    private List<YzPhzCardTypeHuxi> cardTypes;

    public List<YzPhzCardTypeHuxi> getCardTypes() {
        return cardTypes;
    }
    ...
    private YzPhzHandCards handCards = new YzPhzHandCards();
    private YzPhzCardResult huResult;
...
//      }
      ...
      if (this.autoPlay != autoPlay){
			...
			for (Map.Entry<Long,Player> kv:table.getPlayerMap().entrySet()){
			 ...
			 table.addPlayLog(getSeat(), YzPhzConstants.action_tuoguan + "",(autoPlay?1:0) + "");
		}
		this.autoPlay = autoPlay;
...
//				int val=YzPhzCardUtils.loadCardVal(disCard);
...
//		}
        ...
        if (disCardList.size() > 1) {
            ...
            Iterator<YzPhzCardTypeHuxi> iterator = cardTypes.iterator();
            while (iterator.hasNext()) {
                YzPhzCardTypeHuxi type = iterator.next();
                if (type.isHasCard(card)) {
                    if (type.getAction() == YzPhzConstants.action_zai) {
        ...
        if (action == YzPhzConstants.action_ti) {
            ...
            if (copy.size() % 3 == 0) {
                for (int i = 0; i < copy.size() / 3; i++) {
                    ...
                    if (YzPhzCardUtils.loadCardCount(list).size() == 3) {
    ...
    public void addCardType(int action, List<Integer> disCardList) {
        int huxi = YzPhzCardUtils.countHuxi0(new ArrayList<>(disCardList));
        if (action != 0) {
            YzPhzCardTypeHuxi type = new YzPhzCardTypeHuxi();
            type.setAction(action);
            type.setCardIds(disCardList);
            type.setHux(huxi);
            cardTypes.add(type);

            if (action == YzPhzConstants.action_zai || action == YzPhzConstants.action_chouzai || action == YzPhzConstants.action_ti) {
                changeZaihu(huxi);
            } else {
    ...
    public void passVal(int action, int val) {
        if (YzPhzCardUtils.commonCardByVal(val)){
            if (action == YzPhzConstants.action_chi) {
                handCards.NOT_CHI.add(val);
                changeExtend();
                changeTableInfo();
            } else if (action == YzPhzConstants.action_peng) {
...
//    public YzPhzCheckCardBean checkTi() {
//        YzPhzCheckCardBean check = new YzPhzCheckCardBean();
//        check.setSeat(seat);
...
////                list.addAll(temp);
////            }
////            check.setAuto(YzPhzConstants.action_ti, list);
////            check.setTi(true);
//        }
...
//    }
    ...
    public List<Integer> getCalcOverCards() {
        ...
        if (huResult != null && huResult.isCanHu()) {
            for (YzPhzCardMessage temp : huResult.getCardMessageList()) {
    ...
    public boolean isCanDisCard(List<Integer> disCards, Integer nowDisCard, int action) {
        if (disCards != null) {
            if (action == YzPhzConstants.action_zai || action == YzPhzConstants.action_chouzai) {
                return disCards.size() == 2 && handCards.INS.containsAll(disCards);
            } else if (action == YzPhzConstants.action_peng) {
                int val = YzPhzCardUtils.loadCardVal(nowDisCard);
                return YzPhzCardUtils.loadIdsByVal(handCards.INS, val).size() == 2;
            } else if (action == YzPhzConstants.action_ti) {
                int val = YzPhzCardUtils.loadCardVal(nowDisCard);
...
//		if (card != null) {
//
//			YzPhzTable table = getPlayingTable(YzPhzTable.class);
...
//			if (sameList.size() >= 3) {
//				if (isSelfMo) {
//					action = YzPhzConstants.action_ti;
//				} else {
//					action = YzPhzConstants.action_pao;
//				}
//			}
//
...
//	public YzPhzCheckCardBean checkPaoHu() {
//		YzPhzCheckCardBean check = new YzPhzCheckCardBean();
//		check.setSeat(seat);
//
//		YzPhzTable table = getPlayingTable(YzPhzTable.class);
...
//	}
     ...
     * @return
     */
    public YzPhzCheckCardBean checkPaoHu(Integer card0, boolean isSelfMo, boolean checkHu) {
        YzPhzCheckCardBean paohuziCheckCardBean = new YzPhzCheckCardBean();
        paohuziCheckCardBean.setSeat(seat);

        int card = card0 == null ? 0 : card0.intValue();
        if (YzPhzCardUtils.commonCard(card)) {
            List<Integer> list;
            int val = YzPhzCardUtils.loadCardVal(card);
            YzPhzTable table = getPlayingTable(YzPhzTable.class);
            boolean isMoFlag = table.isMoFlag();
            if (isSelfMo && isMoFlag) {
...
//                    result[0] = true;
                    ...
                    if (canHuState == 1 && checkHu) {
                        YzPhzCardResult cardResult = YzPhzHuPaiUtils.huPai(handCards1, 0, table, isSelfMo, userId, table.loadXingCard(false));
...
//                                    result[1] = true;
    ...
    public YzPhzCheckCardBean checkPaoHu2(YzPhzHandCards mHandCards,Integer card0, boolean isSelfMo, boolean checkHu) {
        YzPhzCheckCardBean paohuziCheckCardBean = new YzPhzCheckCardBean();
        paohuziCheckCardBean.setSeat(seat);
...
//                     result[0] = true;
                     paohuziCheckCardBean.setPao(true);
                     YzPhzHandCards handCards1 = mHandCards.copy();
                     list = handCards1.KAN.remove(val);
                     list.add(card);
                     handCards1.PAO.put(val, list);
                     if (canHuState == 1 && checkHu) {
...
//                                      result[1] = true;
    ...
    public YzPhzCheckCardBean checkCard(Integer card, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass) {
        return checkCard(card, isSelfMo, isPassHu, isBegin, isFirstCard, isPass, false, 0, false, -1);
    }

    public YzPhzCheckCardBean checkCard(Integer card0, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass, int huResult) {
        return checkCard(card0, isSelfMo, isPassHu, isBegin, isFirstCard, isPass, false, 0, false, huResult);
    }

    public YzPhzCheckCardBean checkCard(Integer card0, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass, boolean wangPass) {
        return checkCard(card0, isSelfMo, isPassHu, isBegin, isFirstCard, isPass, false, 0, wangPass, -1);
    }

    public YzPhzCheckCardBean checkCard(Integer card0, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass, boolean tiqian, Integer preCard) {
        return checkCard(card0, isSelfMo, isPassHu, isBegin, isFirstCard, isPass, tiqian, preCard, false, -1);
    }

    private int replaceRedCount(YzPhzCardResult cardResult) {
        int count = 0;
        for (String str : cardResult.getReplaceCards()) {
            String[] strs = str.split("\\|");
            for (String s : strs) {
                if (s.length() > 0) {
                    if (YzPhzCardUtils.redCard(Integer.parseInt(s))) {
                        count++;
                        break;
                    }
                }
    ...
    private boolean replaceRed(YzPhzCardResult cardResult) {
     ...
     * @return
     */
    public YzPhzCheckCardBean checkCard(Integer card0, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass, boolean tiqian, Integer preCard, boolean wangPass, int huResult) {
        int card = card0 == null ? 0 : card0.intValue();
        YzPhzCheckCardBean check = new YzPhzCheckCardBean();
        check.setSeat(seat);
        ...
        if (tiqian) {
            if (canHuState == 1) {
//				if (preCard!=null&&YzPhzCardUtils.commonCard(preCard)){
//					int val=YzPhzCardUtils.loadCardVal(preCard);
//					if(nextSeat==seat){
//						if (val<200&&!isPass&&canChi0(preCard)&&handCards.INS.size()>2){
//							if (isMoFlag){
//								check.setChi(true);
//							}else{
//								List<Integer> list=new ArrayList<>(((YzPhzPlayer)table.getSeatMap().get(table.calcFrontSeat(seat))).getHandCards().OUTS);
//                                list.remove(Integer.valueOf(table.getDisposeCard()));
//								boolean bl=true;
//								for (Integer integer:list){
//								    if (val==YzPhzCardUtils.loadCardVal(integer)){
//                                        bl=false;
//                                        break;
//                                    }
//                                }
...
//					}
//
//					if (!isPass&&!handCards.NOT_PENG.contains(val)&&YzPhzCardUtils.canPeng(handCards.INS,preCard)&&handCards.INS.size()>2){
//						check.setPeng(true);
//					}
//				}

                YzPhzCardResult cardResult = YzPhzHuPaiUtils.huPai(handCards, card, table, isSelfMo, userId, table.loadXingCard(tiqian));
                if (cardResult.isCanHu()) {
                    check.setHu(true);
                    check.setCardResult(cardResult);
                    boolean checkHu = true;
                    if ((card > 0 && !YzPhzCardUtils.smallCard(card)) && (cardResult.isWangZha() || cardResult.isWangChuang())) {
                        int huxi = cardResult.getTotalHuxi(false);
                        if (cardResult.isWangZha()) {
                            YzPhzCardMessage cardMessage = null;
                            for (YzPhzCardMessage cm : cardResult.getCardMessageList()) {
                                if (cm.getHuXiEnum() == YzPhzHuXiEnums.TI && YzPhzCardUtils.hasCard(cm.getCards(), val) && YzPhzCardUtils.countCard(cm.getCards(), 201) >= 3) {
                                    cardMessage = cm;
                                    break;
                                }
                            }
                            ...
                            if (cardMessage != null && huxi - cardMessage.loadHuxi() + 9 < table.loadQihuxi()) {
                                ...
                                if ((card > 0 && !YzPhzCardUtils.smallCard(card)) && cardResult.isWangChuang()) {
                                    huxi = cardResult.getChuangCardResult().getTotalHuxi(false);
                                    cardMessage = null;
                                    for (YzPhzCardMessage cm : cardResult.getChuangCardResult().getCardMessageList()) {
                                        if ((cm.getHuXiEnum() == YzPhzHuXiEnums.WEI || cm.getHuXiEnum() == YzPhzHuXiEnums.KAN) && YzPhzCardUtils.hasCard(cm.getCards(), val) && YzPhzCardUtils.countCard(cm.getCards(), 201) >= 2) {
                                            cardMessage = cm;
                                            break;
                                        }
                                    }
                                    ...
                                    if (cardMessage != null && huxi - cardMessage.loadHuxi() + 3 < table.loadQihuxi()) {
                                        ...
                                        if (handCards.WANGS.size() >= 4 && table.loadHuMode() == 2 && !YzPhzCardUtils.wangCard(card) && cardResult.getChuangCardResult().getTotalFan() > 1) {
                                            ...
                                            int red1 = YzPhzCardUtils.redCard(card) ? red0 - 2 : red0;
                                    ...
                                    if (cardResult.isWangChuang() && handCards.WANGS.size() >= 4 && table.loadHuMode() == 2 && !YzPhzCardUtils.wangCard(card) && cardResult.getChuangCardResult().getTotalFan() > 1) {
                                        int redCount = cardResult.getChuangCardResult().getRedTotal();
                                        int baseCount = cardResult.getChuangCardResult().getRedCount();
                                        int red0 = replaceRedCount(cardResult.getChuangCardResult());
                                        int red1 = YzPhzCardUtils.redCard(card) ? red0 - 2 : red0;
                                ...
                                if (cardResult.isWangDiao() && table.loadHuMode() == 2 && cardResult.getDiaoCardResult().getTotalFan() > 1) {
                                    ...
                                    if (YzPhzCardUtils.wangCard(card)) {
                                        if (red0 >= 3) {
                                            red1 = red0 - 2;
                                        } else {
                                        ...
                                        red1 = YzPhzCardUtils.redCard(card) ? red0 - 1 : red0;
                                    }
                    ...
                    if (checkHu) {
                        ...
                        if (cardResult.isWangZha()) {
                            ...
                            if (check.isWangChuang()) {
                                if (handCards.WANGS.size() >= 4 && !YzPhzCardUtils.wangCard(card) && table.loadHuMode() == 2 && cardResult.getChuangCardResult().getTotalFan() > 1) {
                                    int redCount = cardResult.getChuangCardResult().getRedTotal();
                                    int baseCount = cardResult.getChuangCardResult().getRedCount();
                                    int red0 = replaceRedCount(cardResult.getChuangCardResult());
                                    int red1 = YzPhzCardUtils.redCard(card) ? red0 - 2 : red0;
                            ...
                            switch (handCards.WANGS.size()) {
                                ...
                                case 4:
                                    if (redCount + red1 <= 0 || (table.checkRed2Black() && baseCount + red1 < table.loadRed2BlackCount() && redCount >= table.loadRed2BlackCount() + red1)) {
                                    	...
                                    	if(check.isWangZha()){
                                    		//totalFan算了 王炸的番数
                                    		int checkFan = cardResult.getDiaoCardResult().getTotalFan()/8;
                                    		if(checkFan<16){
                                    			check.setWangDiao(false);
                                    		}
                                    	}
                ...
                if (isSelfMo) {
                    if (isMoFlag) {
                        ...
                        if (temp != null&&!temp.contains(card)) {
                            check.setTi(true);
                            check.setAuto(YzPhzConstants.action_ti, new ArrayList<>(temp));
                        } else {
                            ...
                            if (temp != null&&!temp.contains(card)) {
                                ...
                                if (temp != null&&!temp.contains(card)) {
//                                    check.setPao(true);
//                                    check.setAuto(YzPhzConstants.action_pao,new ArrayList<>(temp));
//
                                    ...
                                    temp = YzPhzCardUtils.loadIdsByVal(handCards.INS, val);
                                    if (temp != null && temp.size() == 2&&!temp.contains(card)) {
                                        if (!handCards.NOT_PENG.contains(val) && !handCards.NOT_CHI.contains(val)) {
                                            check.setZai(true);
                                            check.setAuto(YzPhzConstants.action_zai, new ArrayList<>(temp));
                                        } else {
                                            check.setChouZai(true);
                                            check.setAuto(YzPhzConstants.action_chouzai, new ArrayList<>(temp));
                                        }
                                    } else {
...
//
                                ...
                                if (!isPass && !handCards.NOT_PENG.contains(val) && YzPhzCardUtils.canPeng(handCards.INS, card) && handCards.INS.size() > 2) {
                                    check.setPeng(true);
                                }
...
//				if (!isPassHu&&isMoFlag){
//					YzPhzCardResult cardResult=YzPhzHuPaiUtils.huPai(handCards,card,15,isSelfMo);
//					if (cardResult.isCanHu()){
//						if(nextSeat==seat){
//							if (cardResult.isWangZha()){
...
//				}
        ...
        //getCanHuState()==0||
        if(!checkCanhuPass(val, isSelfMo?1:0)) {
        	check.setHu(false);
        }
...
//		LogUtil.msgLog.info("dongzuo list===tableId="+table.getId()+",userId="+userId+",actions="+list);
    ...
    public String buildOutPaiStr() {
        ...
        strBuilder.setLength(0);
        for (YzPhzCardTypeHuxi huxi : cardTypes) {
...
//		this.loseCnt=lostCount;
    ...
    public PlayerInTableRes.Builder buildPlayInTableInfo() {
        return buildPlayInTableInfo(getPlayingTable(YzPhzTable.class),0, false);
    }

    public PlayerInTableRes.Builder buildPlayInTableInfo(YzPhzTable table, long lookUid, boolean isrecover) {
...
//		}
    ...
    public List<Integer> loadFan() {
        List<Integer> list = new ArrayList<>();
        if (huResult != null && huResult.isCanHu()) {
            for (YzPhzFanEnums fanEnums : huResult.getFans()) {
                switch (fanEnums) {
                    case WANG_DIAO:
                        list.add(1);
                        break;
    ...
    public int getTotalFan() {
        if (huResult != null && huResult.isCanHu()) {
           return YzPhzHuPaiUtils.loadMaxFanCount(huResult.getFans());
        }
        return 1;
    }
                ...
                if (StringUtils.isNotBlank(tis)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.TI.put(YzPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
                ...
                if (StringUtils.isNotBlank(paos)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.PAO.put(YzPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
                ...
                if (StringUtils.isNotBlank(kans)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.KAN.put(YzPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
                ...
                if (StringUtils.isNotBlank(weis)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.WEI.put(YzPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
                ...
                if (StringUtils.isNotBlank(weichous)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.WEI_CHOU.put(YzPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
                ...
                if (StringUtils.isNotBlank(pengs)) {
                    ...
                    for (String str : strs) {
                        ...
                        if (temps.length > 1) {
                            ...
                            if (list.size() > 0) {
                                handCards.PENG.put(YzPhzCardUtils.loadCardVal(list.get(0)), list);
                            }
                        }
                    }
                }
    ...
    public void changePoint(int point) {
        ...
        YzPhzTable table = getPlayingTable();
        if(table!=null){
        	table.logTotalPointMsg(this, 0);
        }
...
//		}
        ...
        if (cardTypes.size() > 0) {
            for (YzPhzCardTypeHuxi cth : cardTypes) {
                if (cth.getAction() == 4) {
                    total += cth.getHux();
                }
            }
    ...
    public List<Integer> loadHandCards() {
        ...
        for (YzPhzCardTypeHuxi cth : cardTypes) {
            if (cth.getAction() == YzPhzConstants.action_ti) {
    ...
    public YzPhzCardTypeHuxi convertCardType(List<Integer> list) {
        YzPhzCardTypeHuxi cth = new YzPhzCardTypeHuxi();
        YzPhzCardUtils.sort(list);
        cth.setCardIds(list);
        if (list.size() == 3) {
            int val1 = YzPhzCardUtils.loadCardVal(list.get(0));
            int val2 = YzPhzCardUtils.loadCardVal(list.get(1));
            int val3 = YzPhzCardUtils.loadCardVal(list.get(2));
            List<Integer> list1 = YzPhzCardUtils.loadIdsByVal(list, 201);
            if (list1.size() == 0) {
                if (val1 == val2 && val1 == val3) {
                    cth.setAction(YzPhzConstants.action_zai);
                    cth.setHux(val1 > 100 ? 6 : 3);
                } else if (val1 == 1 && val2 == 2 && val3 == 3) {
                    cth.setAction(YzPhzConstants.action_chi);
                    cth.setHux(3);
                ...
                if (val1 == 201) {
                    ...
                    if (val1 == val2) {
                        cth.setAction(YzPhzConstants.action_zai);
                        cth.setHux(val1 > 100 ? 6 : 3);
                    } else if (YzPhzCardUtils.sameType(list.get(0), list.get(1))) {
                        cth.setAction(YzPhzConstants.action_chi);
            ...
            cth.setAction(YzPhzConstants.action_ti);
            cth.setHux(YzPhzCardUtils.loadCardVal(list.get(0)) > 100 ? 12 : 9);
        }
        return cth;
    }

    public int loadWangHu(YzPhzCardResult cardResult) {
        YzPhzCardResult temp = cardResult != null ? cardResult : huResult;
        int wangHu = -1;
        if (temp.isCanHu()) {
            wangHu = 0;
            for (YzPhzFanEnums fanEnums : temp.getFans()) {
    ...
    public int loadValCount(YzPhzCardResult cardResult, int val) {
        YzPhzCardResult temp = cardResult != null ? cardResult : huResult;
    ...
    public List<PhzHuCards> buildPhzHuCards() {
        ...
        if (huResult != null && huResult.isCanHu()) {
            for (YzPhzCardMessage cm : huResult.getCardMessageList()) {
...
//			System.out.println(huResult.getCardMessageList());
    ...
    public List<PhzHuCards> buildPhzCards(long lookUid) {
        List<PhzHuCards> list = new ArrayList<>();
        for (YzPhzCardTypeHuxi type : cardTypes) {
            if (lookUid != this.userId && type.getAction() == YzPhzConstants.action_zai) {
                // 不是本人并且是栽
     ...
     * @return
     */
    public int calcHuPoint(YzPhzTable table) {
        ...
        total -= table.loadQihuxi();
        
        int totalTun = 2+table.loadScoreTun(total);
        return totalTun;
    }
    ...
    public YzPhzCardResult getHuResult() {
        return huResult;
    }

    public void setHuResult(YzPhzCardResult huResult) {
...
//        int val = YzPhzCardUtils.loadCardVal(id);
//        return !YzPhzCardUtils.hasCard(handCards.OUTS, val) && !handCards.NOT_CHI.contains(val) && YzPhzCardUtils.canChi(handCards.INS, id).size() > 0;
//    }
    ...
    public boolean canChi0(int id) {
        ...
        YzPhzTable table = getPlayingTable(YzPhzTable.class);
        if (table != null) {
            YzPhzPlayer player = (YzPhzPlayer) table.getSeatMap().get(table.calcFrontSeat(seat));
            Set<Integer> set = new HashSet<>(player.getHandCards().OUTS);
            set.remove(Integer.valueOf(id));
            if (YzPhzCardUtils.hasCard(set, val)) {
                return false;
            }
        }
...
//    }
    ...
    public static void loadWanfaPlayers(Class<? extends Player> cls){
        for (Integer integer:wanfaList){
            PlayerManager.wanfaPlayerTypesPut(integer,cls, YzPhzCommandProcessor.getInstance());
        }
    }
    ...
    public Iterable<? extends PhzHuCards> buildNormalPhzHuCards() {
        ...
        if(null!= huResult && huResult.isCanHu() ){
            for (YzPhzCardMessage msg:huResult.getCardMessageList()
                 ) {
                int _hx =0;
               if( msg.getCards().get(0)<=40){
                ...
                list.add(new YzPhzCardTypeHuxi(  msg.getHuXiEnum().getAction(),msg.getCards(), _hx).buildMsg().build());
            }
        }else{
            for (YzPhzCardTypeHuxi type : cardTypes) {
                //修正错误h胡息数
