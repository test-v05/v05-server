...
import com.sy599.game.qipai.lszp.command.PaohuziCommandProcessor;
import com.sy599.game.qipai.lszp.constant.EnumHelper;
...
public class LszpPlayer extends Player {
    // 座位id
    private int seat;
    // 状态
...
//        setFinalPoint(0);
    ...
    public PaohuziHuLack checkPaoHu(PaohzCard card, boolean isSelfMo, boolean canDiHu) {
        ...
        if (card != null) {
            LszpTable table = getPlayingTable(LszpTable.class);
    ...
    public PaohuziCheckCardBean  checkCard(PaohzCard card, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass) {
        ...
        if (card != null) {
            ...
            if (isCanChiPeng && !check.isTi() && !check.isPao() && isCheckChi && !isPass) {
                ...
                LszpPlayer frontPlayer = (LszpPlayer) table.getPlayerBySeat(table.calcFrontSeat(seat));
...
//        res.setFinalPoint(getFinalPoint());
    ...
    public List<PhzHuCards> buildPhzCards(LszpTable table, long lookUid) {
        List<PhzHuCards> list = new ArrayList<>();
        for (CardTypeHuxi type : cardTypes) {
            if (type.getAction() == PaohzDisAction.action_zai) {
                // 不是本人并且是栽
